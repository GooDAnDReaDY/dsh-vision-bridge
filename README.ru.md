...
Крупный релиз стабильности, надёжности и инфраструктуры жизненного цикла.

* **One-Click автообновление из настроек**: добавлен эндпоинт `/api/dsh-vision-bridge/update` и блок управления в карточке настроек (`UpdaterBlock` в `settings.plugin.item`). Поддерживает проверку актуальной версии в npm, индикацию процесса и уведомление о перезапуске с защитой от удалённых вызовов (loopback и CSRF origin gates).
* **Устранение пустых catch и отказоустойчивость**: устранены все 76 неаннотированных пустых блоков catch в ядре и инструментах. Добавлен хелпер `bestEffort(label, fn, fallback)` для безопасных побочных операций, явная передача предупреждений при обработке изображений (`smartOptimizeImage` возвращает `preprocessed: boolean` и `warnings: string[]`), а также прозрачная фиксация деградации.
* **Целостность scoped-имени**: скоуп `@goodandready/dsh-vision-bridge` строго синхронизирован в манифесте `package.json`, патче `cordis.patch.yml`, загрузчике клиента и метаданных ядра `vision-core.js`.
* **Укрепление безопасности настроек**: маршрут изменения настроек переведён на строгий fail-closed валидатор (`isTrustedSettingsRequest`), проверяющий loopback-источник, CSRF-заголовки и Bearer-токены, отклоняя любые подозрительные внешние заголовки.
* **Переменные темы DSH**: стили клиентских карточек и диагностических экранов полностью переведены на токены дизайн-системы DSH (`--dsw-alias-*`), устранены хардкод-цвета.
* **Санитизация публичного релиза**: внедрён reproducible пайплайн публикации (`publish.sh`) и `.gitattributes` фильтры, гарантирующие отсутствие внутренних рабочих материалов в публичном GitHub и npm.

---

## 📝 Изменения в v0.6.1

Релиз качества, расширения тестового покрытия и соответствия политикам pnpm.

* **Соответствие карантину pnpm**: удалён флаг обхода `--config.minimumReleaseAge=0` в автообновлении, обеспечивая строгое соответствие политике безопасности pnpm и штатному кэшированию пакетов.
* **Покрытие инструментов legacy-доменов**: добавлены сквозные тесты выполнения для инструментов обработки изображений (`annotate_image`, `extract_ui_boxes`, `compare_images`, `crop_image`, `rotate_image`, `apply_image_filter`, `read_qr_barcode`, `generate_qr_code`), обеспечив покрытие $\ge 75\%$ по всем доменам (319 успешных тестов).
* **Тестирование DOM и жизненного цикла React**: внедрён компонентный тест-сьют (`test/card-dom.test.js`) для карточки настроек Web UI с эмуляцией React DOM, проверкой монтирования, изоляции состояний каналов и устойчивости Error Boundary.
* **Исправление рантайм-импорта**: устранён дефект отсутствующего импорта `bestEffort` в `lib/index.js` в парсере рамок `parseBbox`.
* **Гигиена публичного дерева**: репозиторий и публичные зеркала санитизированы с полной изоляцией внутренних материалов `docs/` и сервисных скриптов.
