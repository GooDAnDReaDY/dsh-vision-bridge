...
* **Гарды роутов**: `POST /bench`, `POST /batch`, `DELETE /journal`, `DELETE /cache` требуют same-origin, как остальные мутирующие маршруты. `GET /doctor` по умолчанию статический; пробы каналов — только с `?probe=1` (нужен same-origin).
