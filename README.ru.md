...
Релиз надёжности, прозрачности обработки и видимости деградации.

* **Реальное логирование bestEffort**: устранены все фиктивные `/* bestEffort ... */ void err`; сбои фиксируются через `bestEffort(label, fn)` на уровне `console.debug`.
* **Предупреждения о деградации (warnings)**: все инструменты плагина объявляют `warnings: string[]` в схемах `output.schema` и возвращают предупреждения при сбоях парсинга JSON или откатах на сырой вывод.
* **Проброс сбоев предобработки**: ошибки этапов deskew, enhance, stripEXIF и compressImage перехватываются и передаются вызывающим инструментам в виде `warnings`.
* **Тестовый сьют**: добавлен набор тестов `test/issue-314-besteffort-warnings.test.js`, общее покрытие выросло до 329 тестов (104 сьюта, 0 fail).
