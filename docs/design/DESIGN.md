...
### 12. Политика интернационализации и исходного кода (#311)
- **Канонический язык исходников**: все внутренние комментарии, техническая документация и тексты интерфейса пишутся на каноническом английском языке (с локализацией интерфейса через стандартные слоты/словари).
- **Исключения для функциональных данных эвристик разбора ввода**: регулярные выражения и структуры данных, распознающие намерение пользователя на разных языках (включая русский язык, например `genericQuestion = /^(опиши|описать|расскажи|что.*изображ|describe|what.*(image|picture)|what is in)/i` в `lib/index.js`), являются функциональными данными плагина, а не текстом интерфейса или комментариями. Они осознанно сохраняются для обеспечения мультиязычного распознавания намерений пользователей.
