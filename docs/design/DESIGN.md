...
- `attachMaxItems` (число 1–32, по умолчанию 8) и `hideRedundantTools` (флажок, по умолчанию включён) выведены в карточке в группе «Attachments» (#269): первая ограничивает, сколько изображений публикует один вызов `vision_attach_*`, второй скрывает компенсационные инструменты моста у моделей, которые и так видят изображения. Оба поля идут через общий Save, попадают в живой снимок настроек и в `settings.yaml`; значение вне 1–32 отклоняется на `/config` с понятной ошибкой, а не подрезается молча.
...
- **Изоляция по источнику (#258)**: `vision_attach_images` не обрывает вызов из-за одного нечитаемого источника — битый путь или неудачный fetch попадает в `Skipped N: <имя>: <причина>`, остальные источники прикрепляются. Жёстким отказом остаётся срабатывание fetch-политики (включая проверку на каждом хопе редиректа — такое исключение помечено кодом `EFETCH_POLICY` и перебрасывается, а не превращается в пропуск): это защита, а не сбой чтения. Точка инъекции `fetchImpl` у `readRemoteImage`/инструмента позволяет проверять URL-ветку без сети.
...
### 10. Решения по стабильности (#247/#249/#252/#259/#263)
...
- #252: событие `push` ограничено веткой `main` — иначе push в feature-ветку с открытым PR создавал второй прогон того же коммита, а два параллельных прогона гонялись за общий кэш action-репозиториев раннера (`/root/.cache/act`) и один падал на `lstat .../.prettierrc.json` до запуска тестов.
- #263: группа `concurrency` сделана per-ref (`${{ github.workflow }}-${{ github.ref }}`) вместо общей на весь workflow. Общая группа заставляла Gitea отменять ожидающий прогон одного PR при появлении прогона другого PR: коммит получал `failure` без единого запущенного теста и без логов (job `cancelled`, логи 404). Per-ref сериализует повторные прогоны одного ref и не даёт разным PR вытеснять друг друга.

### 11. Продвинутые возможности визуального анализа и телеметрия (#241, #303)
- **Visual Investigation Loop**:
  - `vision_crop`: производит честную нарезку изображения (`cropImageRegion`) по нормализованным (`[ymin, xmin, ymax, xmax]`) или абсолютным координатам и регистрирует производное вложение в сессии, позволяя агенту детально разглядеть мелкие фрагменты.
  - `vision_annotate`: накладывает на изображение цветные рамки (bounding boxes) и текстовые метки для интерактивного указания пользователю на дефекты или UI-элементы.
- **Детерминированные локальные движки без расхода токенов**:
  - `vision_analyze_quality`: детерминированный расчет резкости/смазанности (дисперсия оператора Лапласа), контрастности (RMS), средней яркости и извлечение палитры из 5 доминирующих цветов.
  - `vision_pixel_diff`: попиксельное сравнение двух изображений, вычисление точного процента изменений и ограничивающего прямоугольника дельты (bounding box).
- **Экспорт визуальных артефактов**:
  - `vision_export_artifact`: прямой экспорт таблиц в `.csv` / `.md`, формул в `.tex`, архитектурных диаграмм в `.mmd` в файловую систему проекта.
- **Телеметрия каналов в Web UI**:
  - Эндпоинт `GET /dsh-vision-bridge/telemetry` возвращает живые метрики: последний RTT (мс), min/max/avg задержку, общее число вызовов, процент попадания в кеш.
  - В карточке настроек `settings.plugin.item` отображается блок телеметрии с цветовыми индикаторами задержки (🟢 <800мс, 🟡 <2000мс, 🔴 таймаут).
- **Интернационализация и правила чистоты**:
  - Все новые тексты интерфейса, описания инструментов и ошибки выполнены на английском (`en`) и китайском (`zh`).
  - Словарь для русской локализации выносится в отдельный Issue репозитория `@goodandready/dsh-russian-lang`.
  - Строгий аудит состава пакета: файлы `AGENTS.md`, `index.md`, тесты и служебные планы не включаются в релиз.
