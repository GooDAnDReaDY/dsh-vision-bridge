// lib/updater.js — Host-side one-click updater for @goodandready/dsh-vision-bridge
...
function profileNameFromArgv(argv) {
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (current === '--profile') return argv[index + 1];
    if (current?.startsWith('--profile=')) return current.slice('--profile='.length);
  }
  return undefined;
}
...
async function installExact(target, packageSpec, options) {
  ...
  await new Promise((resolvePromise, reject) => {
    // Note: Do NOT add pnpm release quarantine bypass flag per issue #322, dsh-dsml-artifact-guard#18
    const args = [
  ...
  return host.webServer.register({
    ...
    handler: async (request, response) => {
      try {
        ...
        const isQuarantine = String(error).includes('ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION') ||
          false;
        const userError = isQuarantine
          ? 'The latest version was published recently and is held by pnpm release quarantine; please retry in a few minutes.'
          : 'Plugin update failed; see server logs.';
        json(response, 503, { error: userError, quarantine: isQuarantine });
      }
    },
  });
}
