    ...
    function VisionSection(props) {
          ...
          channels.map((c, i) => {
            ...
            return react_jsx_runtime.jsxs('div', { className: 'vbr-chan', children: [
              ...
              react_jsx_runtime.jsx((() => {
                // #226: unified status badge — key state + circuit state.
                const st = probe[i] && probe[i].circuitState
                const kind = st === 'open' ? 'bad'
                  : st === 'half-open' ? 'warn'
                  : (dot && dot.hasKey ? 'ok' : (dot && dot.key === '' && (c.type === 'ollama' || c.type === 'dsh-catalog') ? 'ok' : 'warn'))
                const label = st === 'open' ? t('circuitOpen')
                  : st === 'half-open' ? t('circuitHalfOpen')
                  : (dot && dot.hasKey ? t('keyOk') : (dot && dot.key === '' && (c.type === 'ollama' || c.type === 'dsh-catalog') ? t('keyOk') : t('keyMissing')))
                const extra = (dot && dot.avgMs) ? ' · ' + dot.avgMs + 'ms' : ''
                return badge(kind, label + extra)
              })()),
            ] }, i)
          }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
      ...
      function apply(ctx) {
        ...
        function VisionCard(props) {
          ...
          react.useEffect(() => {
            ...
            fetch('/dsh-vision-bridge/channels', { cache: 'no-store' })
              ...
              .then((d) => {
                ...
                const ok = d.probe.filter((p) => p.hasKey || p.circuitState === 'closed').length
                setChan({ total, ok })
              })
              .catch(() => {})
            return () => { alive = false }
          ...
          const Boundary = createErrorBoundary('VisionCard')
          return react.createElement('div', { className: 'vbr-card' + (open ? ' vbr-card-open' : '') },
            ...
            open ? react.createElement('div', { className: 'vbr-body' },
              react.createElement(Boundary, null, react.createElement(VisionSection, { ...props, locale }))) : null)
        }
