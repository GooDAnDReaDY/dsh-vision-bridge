...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function api(method, body) {
      ...
      let data = null
      try { data = await res.json() } catch (err) { /* bestEffort parse JSON */ void err }
    ...
    function VisionSection(props) {
        ...
        try {
          ...
          if (scope && typeof scope.subscribe === 'function') {
            off = scope.subscribe(() => {
              ...
              } catch (err) { /* bestEffort snapshot subscriber */ void err }
            })
          }
        } catch (err) { /* bestEffort subscribe */ void err }
        return () => { try { off && off() } catch (err) { /* bestEffort unsubscribe */ void err } }
      }, [props?.ctx])
      ...
      const loadStats = async () => {
        try { const r = await fetch('/dsh-vision-bridge/stats', { cache: 'no-store' }); setStats((await r.json().catch(() => ({}))).channels || {}) } catch (err) { /* bestEffort stats fetch */ void err }
      }
      const runBench = async () => {
        setBenching(true); setBench(null)
        try { const r = await fetch('/dsh-vision-bridge/bench', { method: 'POST' }); setBench((await r.json().catch(() => ({}))).channels || []) } catch (err) { /* bestEffort bench */ void err }
      ...
      function VisionInputControls(props) {
        ...
        const refresh = async () => {
          ...
          } catch (err) { /* bestEffort mode fetch */ void err }
        }
        ...
        const toggleMode = async (e) => {
          ...
          } catch (err) { /* bestEffort mode post */ void err }
        }
