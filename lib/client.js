...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // #44: move from sidebar `settings.section` to Plugins tab `settings.plugin.item`.
      // key must equal settings namespace (NS), not package id. Fallback to old section if slot missing.
      const tryPluginItem = () => {
        try {
