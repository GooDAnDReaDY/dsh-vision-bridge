  ...
  factory: (require) => {
    ...
    function VisionSection(props) {
      ...
      const save = async () => {
        ...
        try {
          const res = await api('POST', { provider, model, mode, describeStrategy, escalation, channelOrderMode, imageMaxWidth, imageMaxHeight, imageQuality, maskPII, maskSystemPaths, stripEXIF, deskew, enhanceImage, selfCheckEnabled: selfCheck, consensusEnabled: consensus })
          if (!res.ok) {
