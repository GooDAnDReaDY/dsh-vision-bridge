    ...
    function VisionSection(props) {
      ...
      function apply(ctx) {
        ...
        try {
          ctx.effect(() => {
            try {
              ...
              console.warn('[dsh-vision-bridge] словарь уже зарегистрирован:', taken && taken.message)
            }
          }, 'dsh-vision-bridge: dictionaries')
        } catch (err) {
          console.warn('[dsh-vision-bridge] ошибка регистрации словаря:', err && err.message)
        }
        function useLocale() {
        ...
        try {
          ...
          console.warn('[dsh-vision-bridge] failed to register settings.plugin.item slot:', err && err.message)
        }
      ...
      const handlePdfUpload = async (file) => {
        const showToast = (msg, isErr) => {
          ...
          t.style.background = isErr ? '#dc2626' : '#2563eb'
          t.style.color = '#fff'
          t.textContent = msg
