...
//
// Top-level Settings section: "Settings → Vision" with a dropdown of vision
// models. State is fetched and persisted via the host's /dsh-vision-bridge
// API, so the card is self-contained and does not depend on the settings-scope
// machinery.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let DICT = RU
    try {
      if (typeof navigator !== 'undefined' && /en\b/i.test(String(navigator.language || ''))) DICT = EN
    } catch {}
    const t = (key, vars) => {
      let s = DICT[key] || key
      if (vars) {
    ...
    function VisionSection() {
      ...
      return react_jsx_runtime.jsxs('div', { className: 'vbr', children: [
        ...
        !loaded
          ? react_jsx_runtime.jsx('p', { className: 'hint', children: '…' })
          : null,
        err
          ? react_jsx_runtime.jsx('p', { className: 'err', children: t('failed') + ': ' + err })
          : null,
        loaded && models.length === 0
          ? react_jsx_runtime.jsx('p', { className: 'err', children: 'В каталоге нет vision-моделей (input: [text, image]) — добавьте vision-модель в Настройки → Модели.' })
          : null,
