...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function VisionSection(props) {
      ...
      const [submittedBlurFaces, setSubmittedBlurFaces] = react.useState(false)
      ...
      const [submittedNsfwFilter, setSubmittedNsfwFilter] = react.useState(false)
      const [tileLargeImages, setTileLargeImages] = react.useState(true)
      const [submittedTileLargeImages, setSubmittedTileLargeImages] = react.useState(true)
      ...
      const load = async () => {
        try {
          ...
          const bf = !!d.blurFaces
          const se = !!d.stripEXIF
          const nf = !!d.nsfwFilter
          const tli = d.tileLargeImages !== false
          const dsk = !!d.deskew
          ...
          setBlurFaces(bf); setSubmittedBlurFaces(bf)
          setStripEXIF(se); setSubmittedStripEXIF(se)
          setNsfwFilter(nf); setSubmittedNsfwFilter(nf)
          setTileLargeImages(tli); setSubmittedTileLargeImages(tli)
      ...
      const dirty = provider !== submittedProvider
        ...
        || blurFaces !== submittedBlurFaces
        || stripEXIF !== submittedStripEXIF
        || nsfwFilter !== submittedNsfwFilter
        || tileLargeImages !== submittedTileLargeImages
      ...
      const save = async () => {
        ...
        try {
          const res = await api('POST', { provider, model, mode, describeStrategy, escalation, channelOrderMode, imageMaxWidth, imageMaxHeight, imageQuality, maskPII, maskSystemPaths, blurFaces, stripEXIF, nsfwFilter, tileLargeImages, deskew, enhanceImage, selfCheckEnabled: selfCheck, consensusEnabled: consensus })
          if (!res.ok) {
          ...
          setSubmittedMaskPII(maskPII); setSubmittedMaskSystemPaths(maskSystemPaths)
          setSubmittedBlurFaces(blurFaces); setSubmittedStripEXIF(stripEXIF)
          setSubmittedNsfwFilter(nsfwFilter); setSubmittedTileLargeImages(tileLargeImages)
      ...
      return react_jsx_runtime.jsxs('div', { className: 'vbr', children: [
        ...
        loaded ? react_jsx_runtime.jsxs('div', { children: [
          ...
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsx('input', { type: 'checkbox', checked: tileLargeImages, onChange: (e) => setTileLargeImages(e.target.checked) }),
            react_jsx_runtime.jsx('label', { children: t('tileLargeImages') }),
          ] }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
      ...
      if (!tryPluginItem()) {
        ctx.slots.inject('settings.section', () =>
          ctx.slots.register(
            ...
            (props) => react.createElement(VisionSection, { ...props, ctx, locale: useLocale() }),
          ),
        )
      }
      ...
      const handlePdfUpload = async (file) => {
        ...
        showToast('📄 Конвертация страниц PDF...', false)
        try {
          ...
          showToast('✅ Загружено ' + d.count + ' стр. из PDF', false)
          ...
          showToast('❌ Ошибка PDF: ' + (e.message || e), true)
        }
      }
      ...
      function VisionInputControls(props) {
        ...
        const modeTitle = currentMode === 'hybrid'
          ? 'Vision: Hybrid (авто-описание + инструменты)'
          : (currentMode === 'llm' ? 'Vision: LLM (только авто-описание)' : 'Vision: Tools (только вызов инструментов)')
        ...
        return react.createElement('div', {
          ...
          react.createElement('button', {
            ...
            title: modeTitle + ' — нажмите для переключения',
            onClick: toggleMode,
          }, '👁️ Vision: ' + currentMode),
          react.createElement('button', {
            type: 'button',
            className: 'vbr-input-btn',
            title: 'Загрузить и конвертировать PDF-документ',
            onClick: onPdfClick,
          }, '📄 +PDF')
        )
      }
