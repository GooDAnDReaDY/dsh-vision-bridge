function bestEffortClient(label, fn, fallback = undefined) {
  try {
    const res = typeof fn === 'function' ? fn() : fn
    if (res && typeof res.catch === 'function') {
      return res.catch((err) => {
        if (typeof console !== 'undefined' && console.debug) {
          console.debug(`[dsh-vision-bridge:client] bestEffort (${label}) caught:`, err?.message || err)
        }
        return fallback
      })
    }
...
//
// Settings card: a "Vision" entry with a dropdown of vision models. Images attached to a conversation are described automatically at the
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // #226: unified UI toolkit — error boundary, slot-registration retry and
    // status badges, patterns adopted from the dsh-clinebot reference card.
    function createErrorBoundary(name) {
      ...
      return class ErrorBoundary extends react.Component {
        ...
        componentDidCatch(error, errorInfo) { console.error('[dsh-vision-bridge] ' + name + ' render error:', error, errorInfo) }
        render() {
          if (this.state.hasError) {
            return react.createElement('div', { className: 'vbr', style: { padding: '14px 16px' } },
              react.createElement('div', { className: 'err', style: { fontWeight: 600, marginBottom: '6px' } }, '⚠️ ' + name + ' UI error:'),
              react.createElement('div', { style: { fontSize: '12px', wordBreak: 'break-all' } }, String(this.state.error?.message || this.state.error)),
              react.createElement('button', { type: 'button', style: { marginTop: '10px' }, onClick: () => this.setState({ hasError: false, error: null }) }, 'Retry'),
            )
          }
      ...
      const tick = () => {
        try {
          ...
          if (slots && typeof slots.inject === 'function' && typeof slots.register === 'function') {
            try { registerFn(slots); return } catch (err) {
              console.warn('[dsh-vision-bridge] failed to register slot ' + slotName + ':', err && err.message)
              return
            }
          }
          if (--left > 0) setTimeout(tick, 500)
          else console.warn('[dsh-vision-bridge] slot ' + slotName + ' never became available')
        } catch (err) {
          console.warn('[dsh-vision-bridge] slot ' + slotName + ' probe failed:', err && err.message)
        }
      }
      tick()
    }
    function badge(kind, text) {
      return react.createElement('span', { className: 'vbr-badge' + (kind ? ' vbr-badge-' + kind : '') }, text)
    }

    // ---------------------------------------------------------------- css
    // #226: unified with the dsh-clinebot reference card — section metrics
    // (18px/20px padding, gap 14, 16px/600 title), status badges, 36px inputs
    // on bg-layer-2 with brand focus ring, state-token toast.
    const css =
      '.vbr{display:flex;flex-direction:column;gap:14px;padding:18px 20px;color:var(--dsw-alias-label-primary);max-width:620px}' +
      ...
      '.vbr select, .vbr input[type=text], .vbr input[type=number]{width:100%;border:1px solid var(--dsw-alias-border-l2);font:inherit;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border-radius:8px;padding:0 12px;font-size:13px;height:36px;box-sizing:border-box;color-scheme:light dark}' +
      '.vbr select:focus, .vbr input:focus{outline:none;border-color:var(--dsw-alias-state-brand-primary)}' +
      '.vbr .row{display:flex;gap:8px;align-items:center;margin-top:8px}' +
      '.vbr-chan{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:10px 12px;margin-top:8px;display:flex;flex-direction:column;gap:6px}' +
      '.vbr-card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;list-style:none}' +
      '.vbr-card-header{appearance:none;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:12px;display:flex;align-items:center;gap:12px;padding:16px 20px}' +
      '.vbr-card-name{color:var(--dsw-alias-label-primary);font-size:16px;font-weight:600;line-height:1.4}' +
      '.vbr-card-description{display:block;color:var(--dsw-alias-label-secondary);font-size:13px;margin-top:2px}' +
      ...
      '.vbr-body{border-top:1px solid var(--dsw-alias-border-l2);margin:0 20px;padding-bottom:12px}' +
      '.vbr-field{display:flex;flex-direction:column;gap:6px;padding:12px 0}' +
      '.vbr button, .vbr select, .vbr input[type=text], .vbr input[type=number]{appearance:none;font:inherit;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:0 12px;font-size:13px;color-scheme:light dark}' +
      '.vbr button, .vbr select{height:36px;cursor:pointer}' +
      '.vbr button:focus-visible, .vbr select:focus-visible{outline:none;border-color:var(--dsw-alias-state-brand-primary)}' +
      ...
      '.vbr input[type=text], .vbr input[type=number]{height:36px}' +
      '.vbr-input-btn{display:inline-flex;align-items:center;gap:4px;height:28px;padding:0 8px;font-size:12px;font-weight:500;border-radius:6px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-input-major);color:var(--dsw-alias-label-secondary);cursor:pointer;transition:all .15s;white-space:nowrap;user-select:none}' +
      ...
      '.vbr-input-btn.active{color:var(--dsw-alias-brand-primary, var(--dsw-alias-state-brand-primary));border-color:var(--dsw-alias-brand-primary, var(--dsw-alias-state-brand-primary))}' +
      '.vbr-badge{font-size:12px;padding:3px 10px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2);display:inline-flex;align-items:center;gap:5px;font-weight:500}' +
      '.vbr-badge-ok{border-color:var(--dsw-alias-state-success-primary);color:var(--dsw-alias-state-success-primary);background:var(--dsw-alias-state-success-bg, var(--dsw-alias-bg-layer-2))}' +
      '.vbr-badge-warn{border-color:var(--dsw-alias-state-warning-primary);color:var(--dsw-alias-state-warning-primary);background:var(--dsw-alias-state-warning-bg, var(--dsw-alias-bg-layer-2))}' +
      '.vbr-badge-bad{border-color:var(--dsw-alias-state-error-primary);color:var(--dsw-alias-state-error-primary);background:var(--dsw-alias-state-error-bg, var(--dsw-alias-bg-layer-2))}' +
      '.vbr-badge-neutral{color:var(--dsw-alias-label-secondary)}' +
      '.vbr-toast{position:fixed;bottom:80px;right:24px;z-index:99999;padding:10px 18px;border-radius:8px;font-size:13px;font-weight:500;box-shadow:var(--dsw-alias-shadow-l2, 0 4px 16px var(--dsw-alias-border-l1));transition:all 0.3s;color:var(--dsw-alias-bg-layer-3)}' +
      '.vbr-toast-ok{background:var(--dsw-alias-state-success-primary)}' +
      '.vbr-toast-err{background:var(--dsw-alias-state-error-primary)}'
    const tagId = 'dsh-vision-bridge/settings-card.module.css'
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css=' + JSON.stringify(tagId) + ']') === null) {
      const tag = document.createElement('style')
      // Canonical ownership marker (#209): neighbor-plugin cleanups and HMR
      // sweep document.head for <style> tags and keep only known
      // data-dsh-plugin labels. Legacy attrs kept for older cores.
      tag.dataset.dshPlugin = 'dsh-vision-bridge'
    ...
    updater_update_now: 'Update to v{version}',
    updater_up_to_date: 'Plugin is up to date',
    updater_failed: 'Update check failed',
    updater_success: 'Update successful. Please restart DSH.',
      title: 'Vision',
      ...
      advanced: 'Advanced',
      secRouting: 'Routing',
      secImage: 'Image quality',
      attachments: 'Attachments',
      attachmentsHint: 'How many images one attach call may publish, and whether the bridge tools are hidden from chat models that already see images.',
      attachMaxItems: 'Max attachments per call',
      attachMaxItemsRange: 'Attachments per call must be a whole number between 1 and 32',
      hideRedundantTools: 'Hide bridge tools on vision models',
      channels: 'Channels',
      ...
      telemetryHint: 'Real-time channel latency (RTT), request volume, and health status.',
      latency: 'Latency',
      requests: 'Requests',
      ...
      degraded: 'Degraded',
      offline: 'Offline',
      qualityAnalysis: 'Image Quality & Blur Detection',
      qualityAnalysisHint: 'Deterministic sharpness (Laplacian variance) and color palette extraction.',
      pixelDiff: 'Pixel Diff Comparison',
      pixelDiffHint: 'Exact pixel-level UI before/after change tracking.',
    ...
    updater_update_now: '立即更新至 v{version}',
    updater_up_to_date: '已是最新版本',
    updater_failed: '检查更新失败',
    updater_success: '更新成功，请重启 DSH。',
      title: '视觉 (Vision)',
      subtitle: '聊天中的图片将由您在此处选择的视觉模型处理。若两项均留空，则自动从目录中选择首个具备视觉能力的模型。',
      provider: '视觉服务商',
      model: '视觉模型',
      auto: '自动',
      mode: '工作模式',
      ...
      describeDeep: '深度解析',
      escalation: '升级策略',
      escalationSimpleOnly: '仅简单升级',
      escalationAlwaysFull: '始终完整解析',
      save: '保存配置',
      saved: '已保存',
      saving: '保存中…',
      attachments: '附件与工具',
      attachmentsHint: '单次调用允许发布的最大图片数，以及是否向原生支持视觉的模型隐藏桥接工具。',
      attachMaxItems: '单次调用最大附件数',
      attachMaxItemsRange: '单次调用附件数必须是 1 到 32 之间的整数',
      hideRedundantTools: '向视觉模型隐藏多余桥接工具',
      channels: '通道配置',
      channelsHint: '额外视觉端点 (兼容 OpenAI, Ollama, 自定义)。留空则自动从目录中选择。',
      addChannel: '添加通道',
      remove: '删除',
      testVision: '测试连通性',
      testing: '测试中…',
      ...
      channelOrderMode: '通道优选顺序',
      deskew: '图像自动纠偏',
      enhanceImage: '画质增强优化',
      selfCheck: '视觉自检验证',
      consensus: '多模型交叉共识 (防幻觉过滤)',
      maskPII: '提示词脱敏 (屏蔽个人信息)',
      maskSystemPaths: '响应脱敏 (屏蔽本地路径与IP)',
      stripEXIF: '移除 EXIF 隐私元数据',
      security: '安全与隐私',
      channelOrderManual: '手动顺序',
      channelOrderAutoLatency: '按时延自动优选',
      imageMaxWidth: '最大宽度 (px)',
      imageMaxHeight: '最大高度 (px)',
      imageQuality: '压缩质量 (1-100)',
      circuitState: '熔断状态',
      circuitOpen: '已阻断',
      circuitHalfOpen: '探测恢复中',
      telemetry: '通道实时遥测',
      telemetryHint: '各通道实时往返时延 (RTT)、请求吞吐量与健康状态监控。',
      latency: '时延',
      requests: '请求数',
      successRate: '成功率',
      healthy: '正常',
      degraded: '高时延',
      offline: '离线',
      qualityAnalysis: '画质分析与模糊检测',
      qualityAnalysisHint: '基于拉普拉斯方差的边缘锐度分析与 5 色主色调提取。',
      pixelDiff: '像素级差异对比',
      pixelDiffHint: '界面修改前后精确像素级对比与变更范围检测。',
    }

    function useActiveLocale(ctx) {
      return react.useSyncExternalStore(
        react.useMemo(() => (cb) => {
          const loc = ctx && (ctx.get ? ctx.get('locale') : ctx.locale)
          return loc && typeof loc.subscribe === 'function' ? loc.subscribe(cb) : () => {}
        }, [ctx]),
        react.useCallback(() => {
          const loc = ctx && (ctx.get ? ctx.get('locale') : ctx.locale)
    ...
    function makeT(DICT, fallbackKeys, activeLang) {
      const activeDict = activeLang === 'zh' ? zh : (DICT || en)
      return (key, vars) => {
        let s = (activeDict && activeDict[key]) || (DICT && DICT[key]) || (fallbackKeys && fallbackKeys[key]) || key
        if (vars) {
    ...
    function _unused_makeT(DICT, fallbackKeys) {
      return (key, vars) => {
    ...
    async function api(method, body) {
      ...
      let data = null
      data = await bestEffortClient('client.parseJson', () => res.json(), {})
    ...
    function UpdaterBlock({ t }) {
      ...
      const checkUpdate = react.useCallback(async () => {
        ...
        try {
          const res = await fetch('/api/dsh-vision-bridge/update', {
      ...
      const currentVer = status?.currentVersion || '0.5.37'
      const latestVer = status?.latestVersion
      const updateAvailable = !!status?.updateAvailable && !!latestVer && latestVer !== currentVer

      return react_jsx_runtime.jsxs('div', {
        className: 'vbr-section vbr-updater-card',
        style: { marginTop: '16px', paddingTop: '12px', borderTop: '1px solid var(--dsw-alias-border-l2)' },
        children: [
          react_jsx_runtime.jsxs('div', {
            style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' },
            children: [
              react_jsx_runtime.jsx('strong', { style: { color: 'var(--dsw-alias-label-primary)' }, children: t('updater_title') }),
              react_jsx_runtime.jsx('button', {
                type: 'button',
                className: 'vbr-btn',
                disabled: loading || updating,
                onClick: checkUpdate,
                children: loading ? t('updater_checking') : t('updater_check'),
              }),
            ],
          }),
          react_jsx_runtime.jsxs('div', {
            ...
            children: [
              react_jsx_runtime.jsxs('div', {
                children: [
                  react_jsx_runtime.jsx('span', { style: { color: 'var(--dsw-alias-label-secondary)' }, children: t('updater_current') + ': ' }),
                  react_jsx_runtime.jsx('code', { style: { color: 'var(--dsw-alias-label-primary)' }, children: `v${currentVer}` }),
                ],
              }),
              latestVer
                ? react_jsx_runtime.jsxs('div', {
                    children: [
                      react_jsx_runtime.jsx('span', { style: { color: 'var(--dsw-alias-label-secondary)' }, children: t('updater_latest') + ': ' }),
                      react_jsx_runtime.jsx('code', {
                        style: { color: updateAvailable ? 'var(--dsw-alias-state-warning-primary)' : 'var(--dsw-alias-state-success-primary)' },
                        children: `v${latestVer}`,
                      }),
                    ],
                  })
                : null,
          ...
          updateAvailable
            ? react_jsx_runtime.jsxs('div', {
                style: { marginTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'var(--dsw-alias-state-warning-bg, var(--dsw-alias-bg-layer-2))', borderRadius: '6px', border: '1px solid var(--dsw-alias-border-l1)' },
                children: [
                  react_jsx_runtime.jsx('span', { style: { fontSize: '12px', color: 'var(--dsw-alias-state-warning-primary)', fontWeight: 500 }, children: `New release available: v${latestVer}` }),
                  react_jsx_runtime.jsx('button', {
                    type: 'button',
                    className: 'primary',
                    disabled: updating,
                    onClick: onUpdateNow,
                    children: updating ? t('updater_updating') : t('updater_update_now').replace('{version}', latestVer),
                  }),
                ],
              })
            : null,
    ...
    function VisionSection(props) {
      ...
      const [submittedChannelOrderMode, setSubmittedChannelOrderMode] = react.useState('manual')
      const [maskPII, setMaskPII] = react.useState(false)
      const [submittedMaskPII, setSubmittedMaskPII] = react.useState(false)
      const [maskSystemPaths, setMaskSystemPaths] = react.useState(false)
      const [submittedMaskSystemPaths, setSubmittedMaskSystemPaths] = react.useState(false)
      const [stripEXIF, setStripEXIF] = react.useState(false)
      const [submittedStripEXIF, setSubmittedStripEXIF] = react.useState(false)
      const [deskew, setDeskew] = react.useState(false)
      const [submittedDeskew, setSubmittedDeskew] = react.useState(false)
      const [enhanceImage, setEnhanceImage] = react.useState(false)
      const [submittedEnhanceImage, setSubmittedEnhanceImage] = react.useState(false)
      const [selfCheck, setSelfCheck] = react.useState(true)
      const [consensus, setConsensus] = react.useState(false)
      const [submittedConsensus, setSubmittedConsensus] = react.useState(false)
      const [submittedSelfCheck, setSubmittedSelfCheck] = react.useState(true)
      const [imageMaxWidth, setImageMaxWidth] = react.useState(1920)
      const [submittedImageMaxWidth, setSubmittedImageMaxWidth] = react.useState(1920)
      const [imageMaxHeight, setImageMaxHeight] = react.useState(1080)
      const [submittedImageMaxHeight, setSubmittedImageMaxHeight] = react.useState(1080)
      const [imageQuality, setImageQuality] = react.useState(80)
      const [submittedImageQuality, setSubmittedImageQuality] = react.useState(80)
      const [attachMaxItems, setAttachMaxItems] = react.useState(8)
      const [submittedAttachMaxItems, setSubmittedAttachMaxItems] = react.useState(8)
      const [hideRedundantTools, setHideRedundantTools] = react.useState(true)
      const [submittedHideRedundantTools, setSubmittedHideRedundantTools] = react.useState(true)
      const [status, setStatus] = react.useState(null)
      ...
      const load = async () => {
        try {
          ...
          const p = d.provider || ''
          const m = d.model || ''
          const md = d.mode || 'hybrid'
          const ds = d.describeStrategy || 'auto'
          const esc = d.escalation || 'simple-only'
          const com = d.channelOrderMode || 'manual'
          const pii = !!d.maskPII
          const sp = !!d.maskSystemPaths
          const se = !!d.stripEXIF
          const dsk = !!d.deskew
          const enh = !!d.enhanceImage
          const sc = d.selfCheckEnabled !== false
          const cs = !!d.consensusEnabled
          const mw = d.imageMaxWidth || 1920
          const mh = d.imageMaxHeight || 1080
          const iq = d.imageQuality || 80
          const ami = Number(d.attachMaxItems) || 8
          const hrt = d.hideRedundantTools !== false

          setProvider(p); setSubmittedProvider(p)
          setModel(m); setSubmittedModel(m)
          setMode(md); setSubmittedMode(md)
          setDescribeStrategy(ds); setSubmittedDescribeStrategy(ds)
          setEscalation(esc); setSubmittedEscalation(esc)
          setChannelOrderMode(com); setSubmittedChannelOrderMode(com)
          setMaskPII(pii); setSubmittedMaskPII(pii)
          setMaskSystemPaths(sp); setSubmittedMaskSystemPaths(sp)
          setStripEXIF(se); setSubmittedStripEXIF(se)
          setDeskew(dsk); setSubmittedDeskew(dsk)
          setEnhanceImage(enh); setSubmittedEnhanceImage(enh)
          setSelfCheck(sc); setSubmittedSelfCheck(sc)
          setConsensus(cs); setSubmittedConsensus(cs)
          setImageMaxWidth(mw); setSubmittedImageMaxWidth(mw)
          setImageMaxHeight(mh); setSubmittedImageMaxHeight(mh)
          setImageQuality(iq); setSubmittedImageQuality(iq)
          setAttachMaxItems(ami); setSubmittedAttachMaxItems(ami)
          setHideRedundantTools(hrt); setSubmittedHideRedundantTools(hrt)
      ...
      // Reactive binding to settingsScope (#191)
      react.useEffect(() => {
        let off = null
        try {
          const scope = props?.ctx?.settingsScope && typeof props.ctx.settingsScope.bind === 'function'
            ? props.ctx.settingsScope.bind({ namespace: NS })
            : null
          if (scope && typeof scope.subscribe === 'function') {
            off = scope.subscribe(() => {
              try {
                const s = scope.getSnapshot ? scope.getSnapshot() : null
                if (s && s.value) {
                  const v = s.value
                  if (typeof v.visionProvider === 'string') setProvider(v.visionProvider)
                  if (typeof v.visionModel === 'string') setModel(v.visionModel)
                  if (typeof v.mode === 'string') setMode(v.mode)
                }
              } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge:client] snapshot subscriber error:', err?.message || err) }
            })
          }
        } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge:client] subscribe error:', err?.message || err) }
        return () => { bestEffortClient('client.unsubscribe', () => off && off()) }
      }, [props?.ctx])
      ...
      const loadStats = async () => {
        await bestEffortClient('client.statsFetch', async () => { const r = await fetch('/dsh-vision-bridge/stats', { cache: 'no-store' }); setStats((await r.json().catch(() => ({}))).channels || {}) })
      }
      const runBench = async () => {
        setBenching(true); setBench(null)
        try {
          await bestEffortClient('client.benchFetch', async () => { const r = await fetch('/dsh-vision-bridge/bench', { method: 'POST' }); setBench((await r.json().catch(() => ({}))).channels || []) })
      ...
      const groupLabels = { cloud: 'Cloud', local: 'Local', custom: 'Custom endpoints' }
      ...
      const dirty = provider !== submittedProvider
        ...
        || channelOrderMode !== submittedChannelOrderMode
        || maskPII !== submittedMaskPII
        || maskSystemPaths !== submittedMaskSystemPaths
        || stripEXIF !== submittedStripEXIF
        || deskew !== submittedDeskew
        || enhanceImage !== submittedEnhanceImage
        || selfCheck !== submittedSelfCheck
        || consensus !== submittedConsensus
        || imageMaxWidth !== submittedImageMaxWidth
        || imageMaxHeight !== submittedImageMaxHeight
        || imageQuality !== submittedImageQuality
        || attachMaxItems !== submittedAttachMaxItems
        || hideRedundantTools !== submittedHideRedundantTools
      ...
      const save = async () => {
        ...
        // #288: validate before writing — the snapshot is updated only after
        // the route confirms the write, so an invalid value never persists.
        const cap = Number(attachMaxItems)
        if (!Number.isInteger(cap) || cap < 1 || cap > 32) {
          setStatus({ kind: 'err', msg: t('attachMaxItemsRange') })
          return
        }
        setSaving(true); setStatus(null)

        try {
          const res = await api('POST', { provider, model, mode, describeStrategy, escalation, channelOrderMode, imageMaxWidth, imageMaxHeight, imageQuality, attachMaxItems, hideRedundantTools, maskPII, maskSystemPaths, stripEXIF, deskew, enhanceImage, selfCheckEnabled: selfCheck, consensusEnabled: consensus })
          if (!res.ok) {
            const m = (res.data && (res.data.error || res.data.message)) || (res.status + '')
            throw new Error(typeof m === 'string' ? m : JSON.stringify(m))
          }
          // #288: the settings snapshot is updated only after the route confirms
          // the write, so a rejected value can never reach the snapshot.
          const scope = props?.ctx?.settingsScope && typeof props.ctx.settingsScope.bind === 'function'
            ? props.ctx.settingsScope.bind({ namespace: NS })
            : null
          ...
          setSubmittedChannelOrderMode(channelOrderMode)
          setSubmittedMaskPII(maskPII); setSubmittedMaskSystemPaths(maskSystemPaths); setSubmittedStripEXIF(stripEXIF)
          setSubmittedDeskew(deskew); setSubmittedEnhanceImage(enhanceImage)
          setSubmittedSelfCheck(selfCheck); setSubmittedConsensus(consensus); setSubmittedImageMaxWidth(imageMaxWidth)
          setSubmittedImageMaxHeight(imageMaxHeight); setSubmittedImageQuality(imageQuality)
          setSubmittedAttachMaxItems(attachMaxItems); setSubmittedHideRedundantTools(hideRedundantTools)
          setStatus({ kind: 'ok', msg: t('saved') })
        } catch (e) {
          setStatus({ kind: 'err', msg: String((e && e.message) || e) || t('failed') })
        } finally { setSaving(false) }
      ...
      return react_jsx_runtime.jsxs('div', { className: 'vbr', children: [
        ...
        loaded ? react_jsx_runtime.jsx('h2', { children: t('secRouting') }) : null,
        loaded ? react_jsx_runtime.jsxs('div', { children: [
        ...
        loaded ? react_jsx_runtime.jsx('h2', { children: t('secImage') }) : null,
        loaded ? react_jsx_runtime.jsxs('div', { children: [
        ...
        loaded ? react_jsx_runtime.jsxs('div', { children: [
          react_jsx_runtime.jsx('h2', { children: t('attachments') }),
          react_jsx_runtime.jsx('p', { className: 'hint', children: t('attachmentsHint') }),
          react_jsx_runtime.jsxs('div', { children: [
            react_jsx_runtime.jsx('label', { children: t('attachMaxItems') }),
            react_jsx_runtime.jsx('input', { type: 'number', min: '1', max: '32', value: attachMaxItems, onChange: (e) => setAttachMaxItems(Number(e.target.value)) }),
          ] }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsx('input', { type: 'checkbox', checked: hideRedundantTools, onChange: (e) => setHideRedundantTools(e.target.checked) }),
            react_jsx_runtime.jsx('label', { children: t('hideRedundantTools') }),
          ] }),
        ] }) : null,
        loaded ? react_jsx_runtime.jsxs('div', { children: [
          ...
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsx('input', { type: 'checkbox', checked: deskew, onChange: (e) => setDeskew(e.target.checked) }),
            react_jsx_runtime.jsx('label', { children: t('deskew') }),
          ] }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsx('input', { type: 'checkbox', checked: enhanceImage, onChange: (e) => setEnhanceImage(e.target.checked) }),
            react_jsx_runtime.jsx('label', { children: t('enhanceImage') }),
          ] }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsx('input', { type: 'checkbox', checked: selfCheck, onChange: (e) => setSelfCheck(e.target.checked) }),
            react_jsx_runtime.jsx('label', { children: t('selfCheck') }),
          ] }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsx('input', { type: 'checkbox', checked: consensus, onChange: (e) => setConsensus(e.target.checked) }),
            react_jsx_runtime.jsx('label', { children: t('consensus') }),
          ] }),
        ] }) : null,
        !loaded
        ...
        loaded && provider ? react_jsx_runtime.jsxs('div', { children: [
          ...
          react_jsx_runtime.jsxs('select', {
            ...
            children: modelsForProvider.length > 0 ? [
              react_jsx_runtime.jsx('option', { value: '', children: t('auto') }),
              modelsForProvider.map((m) => react_jsx_runtime.jsx('option', { value: m.model, children: m.vision ? m.model + ' (vision)' : m.model }, m.model)),
            ] : [
        ...
        loaded ? react_jsx_runtime.jsxs('div', { children: [
          ...
          channels.map((c, i) => {
            const dot = probe[i]
            // #226-review: badge kind/label computed as plain consts — never
            // pass a ready element into jsx() as its type.
            const st = dot && dot.circuitState
            const badgeKind = st === 'open' ? 'bad'
              : st === 'half-open' ? 'warn'
              : (dot && dot.hasKey ? 'ok' : (dot && dot.key === '' && (c.type === 'ollama' || c.type === 'dsh-catalog') ? 'ok' : 'warn'))
            const badgeLabel = st === 'open' ? t('circuitOpen')
              : st === 'half-open' ? t('circuitHalfOpen')
              : (dot && dot.hasKey ? t('keyOk') : (dot && dot.key === '' && (c.type === 'ollama' || c.type === 'dsh-catalog') ? t('keyOk') : t('keyMissing')))
            const badgeExtra = (dot && dot.avgMs) ? ' · ' + dot.avgMs + 'ms' : ''
            return react_jsx_runtime.jsxs('div', { className: 'vbr-chan', children: [
              ...
              badge(badgeKind, badgeLabel + badgeExtra),
            ] }, i)
          }),
          react_jsx_runtime.jsxs('div', { className: 'row', children: [
            react_jsx_runtime.jsxs('select', {
          ...
          react_jsx_runtime.jsx(UpdaterBlock, { t }),
        ] }) : null,
      ] })
    }
      ...
      function apply(ctx) {
        ...
        if (typeof window !== 'undefined' && typeof document !== 'undefined' && !window.__vbr_lightbox_installed) {
          ...
          const openLightbox = (imgSrc, altText) => {
            ...
            overlay.style.cssText = 'position:fixed;inset:0;z-index:999999;background:var(--dsw-alias-bg-mask, var(--dsw-alias-bg-layer-1));backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;cursor:zoom-out;padding:24px;animation:vbrFadeIn 0.2s ease-out;';

            const img = document.createElement('img');
            img.src = imgSrc;
            img.alt = altText || 'Preview';
            img.style.cssText = 'max-width:92vw;max-height:90vh;border-radius:8px;box-shadow:var(--dsw-alias-shadow-l3, 0 12px 48px var(--dsw-alias-border-l2));object-fit:contain;cursor:default;';

            const closeBtn = document.createElement('button');
            closeBtn.textContent = '✕';
            closeBtn.style.cssText = 'position:absolute;top:20px;right:24px;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-primary);border:none;width:36px;height:36px;border-radius:50%;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;';
        ...
        // #222: Safe registration with try/catch to prevent double-apply crashing the client half.
        try {
          ctx.effect(() => {
            try {
              const localeService = ctx.get ? ctx.get('locale') : ctx.locale
              if (localeService && typeof localeService.register === 'function') {
                localeService.register(NS, { en, zh })
              } else if (ctx.locale && typeof ctx.locale.register === 'function') {
                ctx.locale.register(NS, { en, zh })
              }
            } catch (taken) {
              console.warn('[dsh-vision-bridge] dictionary already registered:', taken && taken.message)
            }
          }, 'dsh-vision-bridge: dictionaries')
        } catch (err) {
          console.warn('[dsh-vision-bridge] dictionary registration error:', err && err.message)
        }
        function useLocale() {
          return useActiveLocale(ctx)
        }
        // #44 / #222: collapsible card in Plugins tab `settings.plugin.item` (like Model Sync/Spendmeter).
        // key must equal settings namespace (NS). Redundant top-level settings.section removed.
        function VisionCard(props) {
          const locale = useLocale(); const t = makeT(en, en); const [open, setOpen] = react.useState(false)
          // #226: card-level channel health badge from /channels probe.
          const [chan, setChan] = react.useState(null)
          react.useEffect(() => {
            let alive = true
            fetch('/dsh-vision-bridge/channels', { cache: 'no-store' })
              .then((r) => r.json())
              .then((d) => {
                if (!alive || !d || !Array.isArray(d.probe)) return
                const total = d.probe.length
                // #226-review: aggregate mirrors the per-row logic — a keyless-type
          // channel (ollama/dsh-catalog) without a stored failure state is ok.
          const ok = d.probe.filter((p) => p.circuitState !== 'open' && (p.hasKey || (p.key === '' && (p.type === 'ollama' || p.type === 'dsh-catalog')))).length
                setChan({ total, ok })
              })
              .catch(() => {})
            return () => { alive = false }
          }, [])
          const kind = !chan ? '' : chan.total === 0 ? 'neutral' : (chan.ok === chan.total ? 'ok' : (chan.ok > 0 ? 'warn' : 'bad'))
          const badgeText = !chan ? '' : (chan.total === 0 ? t('auto') : 'ch ' + chan.ok + '/' + chan.total)
          return react.createElement('div', { className: 'vbr-card' + (open ? ' vbr-card-open' : '') },
            react.createElement('button', { type: 'button', className: 'vbr-card-header', 'aria-expanded': open, onClick: () => setOpen((v) => !v) },
              ...
              badgeText ? badge(kind, badgeText) : null,
              react.createElement('span', { className: 'vbr-card-chevron', 'aria-hidden': 'true' }, react.createElement(Chevron))),
            // #226-review: no inner boundary here — VisionCard already renders
            // inside the VisionSettings boundary created once in apply().
            open ? react.createElement('div', { className: 'vbr-body' }, react.createElement(VisionSection, { ...props, locale })) : null)
        }
        const Boundary = createErrorBoundary('VisionSettings')
        registerSlotWhenReady(ctx, 'settings.plugin.item', (slots) =>
          slots.inject('settings.plugin.item', () =>
            slots.register(
              ...
              (props) => react.createElement(Boundary, null, react.createElement(VisionCard, props)),
            ),
          ),
        )
      // ------------------------------------------------------- conversation.input.right / composer bar
      ...
      const handlePdfUpload = async (file) => {
        const showToast = (msg, isErr) => {
          ...
          if (!t) {
            ...
            t.id = 'vbr-pdf-toast'
            t.className = 'vbr-toast'
            document.body.appendChild(t)
          }
          // #226: state-token colors instead of hardcoded hex.
          t.className = 'vbr-toast ' + (isErr ? 'vbr-toast-err' : 'vbr-toast-ok')
          t.textContent = msg
          setTimeout(() => { if (t) t.remove() }, 4000)
        }

        showToast('📄 Converting PDF pages…', false)
        try {
          ...
          if (!res.ok || !d.ok) throw new Error(d.error || 'PDF conversion failed')
          showToast('✅ Loaded ' + d.count + ' PDF pages', false)
          ...
          showToast('❌ PDF error: ' + (e.message || e), true)
        }
      }
      ...
      function VisionInputControls(props) {
        ...
        const refresh = async () => {
          try {
            ...
            if (typeof d.mode === 'string' && MODES.includes(d.mode)) {
              setCurrentMode(d.mode)
            }
          } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge:client] mode fetch error:', err?.message || err) }
        }
        ...
        const toggleMode = async (e) => {
          ...
          } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge:client] mode post error:', err?.message || err) }
        }

        const onPdfClick = (e) => {
          e?.preventDefault()
          e?.stopPropagation()
          let fileInput = document.getElementById('vbr-pdf-file-input')
          if (!fileInput) {
            fileInput = document.createElement('input')
            fileInput.id = 'vbr-pdf-file-input'
            fileInput.type = 'file'
        ...
        const modeTitle = currentMode === 'hybrid'
          ? 'Vision: Hybrid (auto description + tools)'
          : (currentMode === 'llm' ? 'Vision: LLM (auto description only)' : 'Vision: Tools (tool calls only)')

        return react.createElement('div', {
          style: { display: 'inline-flex', alignItems: 'center', gap: '6px', margin: '0 4px' },
        },
          react.createElement('button', {
            type: 'button',
            className: 'vbr-input-btn' + (currentMode === 'hybrid' ? ' active' : ''),
            title: modeTitle + ' — click to switch',
            onClick: toggleMode,
          }, '👁️ Vision: ' + currentMode),
          react.createElement('button', {
            type: 'button',
            className: 'vbr-input-btn',
            title: 'Upload and convert a PDF document',
            onClick: onPdfClick,
          }, '📄 +PDF')
        )
      }

      // Slot 1: conversation.input.right (alongside subscriptions and voice)
      const InputBoundary = createErrorBoundary('VisionComposer')
      registerSlotWhenReady(ctx, 'conversation.input.right', (slots) =>
        slots.inject('conversation.input.right', () =>
          slots.register(
            {
              name: 'conversation.input.right',
              id: '@goodandready/dsh-vision-bridge-mode',
              order: 7,
              locale: NS,
              label: () => 'Vision Mode',
            },
            (props) => react.createElement(InputBoundary, null, react.createElement(VisionInputControls, { ...props })),
          ),
        ),
      )
    }
