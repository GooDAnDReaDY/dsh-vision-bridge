...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css =
      ...
      '.vbr-card{border:1px solid var(--dsw-alias-border-l2);border-radius:8px;overflow:hidden}' +
      '.vbr-card-header{display:flex;align-items:center;justify-content:space-between;width:100%;padding:12px 16px;background:var(--dsw-alias-bg-subtle,transparent);border:0;text-align:left;cursor:pointer}' +
      '.vbr-card-head-text{display:flex;flex-direction:column;gap:2px}' +
      '.vbr-card-name{font-size:14px;font-weight:600}' +
      '.vbr-card-description{font-size:12px;color:var(--dsw-alias-label-secondary)}' +
      '.vbr-card-chevron{font-size:12px}' +
      '.vbr-card-body{padding:16px 20px;border-top:1px solid var(--dsw-alias-border-l2)}' +
    ...
    function VisionSection(props) {
      ...
      const applyPreset = async (kind) => {
        ...
        if (res.ok) setStatus({ kind: 'ok', msg: t('saved') })
        else setStatus({ kind: 'err', msg: (res.data && res.data.error) || 'preset failed' })
      }
      ...
      const providers = Array.from(new Set(allModels.map((m) => String(m.provider))))
      ...
      return react_jsx_runtime.jsxs('div', { className: 'vbr', children: [
        ...
        loaded && models.length > 0 ? react_jsx_runtime.jsxs('div', { children: [
          ...
          react_jsx_runtime.jsxs('select', {
            ...
            children: [
              ...
              filteredProviders.map((p) => react_jsx_runtime.jsx('option', { value: p, children: p }, p)),
            ],
          }),
        ] }) : null,
        ...
        bench ? react_jsx_runtime.jsx('p', { className: 'hint', children: bench.map((b) => `${b.key}: ${b.ok ? b.latencyMs + 'ms' : 'fail'}`).join(', ') }) : null,
      ...
      function VisionCard(props) {
        ...
        return react.createElement('div', { className: 'vbr-card' + (open ? ' vbr-card-open' : '') },
          react.createElement('button', { type: 'button', className: 'vbr-card-header', 'aria-expanded': open, onClick: () => setOpen((v) => !v) },
            ...
            react.createElement('span', { className: 'vbr-card-chevron', 'aria-hidden': 'true' }, open ? '▴' : '▾')),
          open ? react.createElement('div', { className: 'vbr-card-body' }, react.createElement(VisionSection, { ...props, locale })) : null)
      }
      const tryPluginItem = () => {
        try {
