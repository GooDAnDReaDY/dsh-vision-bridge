  ...
  factory: (require) => {
    ...
    function VisionSection(props) {
      ...
      const save = async () => {
        ...
        setSaving(true); setStatus(null)
        try {
          const res = await api('POST', { provider, model, mode, describeStrategy, escalation, channelOrderMode, imageMaxWidth, imageMaxHeight, imageQuality, maskPII, maskSystemPaths, blurFaces, stripEXIF, nsfwFilter, tileLargeImages, deskew, enhanceImage })
          if (!res.ok) {
