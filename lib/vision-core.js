...
/**
 * Executes an action safely, capturing any error and logging at debug level if available.
 * Returns the fallback value if the action throws or rejects.
 */
export function bestEffort(label, fn, fallback = undefined) {
  let op = fn
  let tag = label
  let defVal = fallback
  if (typeof tag === 'function') {
    op = label
    tag = typeof fn === 'string' ? fn : ''
    defVal = fallback
  }
  try {
    const res = typeof op === 'function' ? op() : op
    if (res && typeof res.catch === 'function') {
      return res.catch((err) => {
        if (tag && typeof console !== 'undefined' && console.debug) {
          console.debug(`[dsh-vision-bridge] bestEffort (${tag}) caught:`, err?.message || err)
        }
        return defVal
      })
    }
...
export function isTrustedSettingsRequest(request) {
  ...
  // Cross-site requests are unconditionally forbidden
  if (headers['sec-fetch-site'] === 'cross-site') return false

  // Reject mismatched origin and host when both are provided (CSRF protection)
  ...
  // 2. Cookie containing authentication token
  const cookie = headers['cookie']
  if (typeof cookie === 'string' && (cookie.includes('token=') || cookie.includes('dsh-auth') || cookie.includes('dsh_token='))) {
    return true
  }

  // 3. Browser same-origin navigation / fetch
  if (headers['sec-fetch-site'] === 'same-origin') {
    return true
  }

  // 4. Socket loopback remote address
...
export async function resolveChannelApiKey(channel, resolveCred, env = process.env) {
  ...
  if (typeof resolveCred === 'function') {
    try { key = String((await resolveCred(name)) || '') } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] resolveCred failed:', err?.message || err) }
  }
  ...
  try {
    ...
    if (hasTess) {
      ...
      try {
        ...
        try { unlinkSync(imgPath) } catch (err) { /* bestEffort cleanup */ void err }
      }
    }
  } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] runLocalOCR failed:', err?.message || err) }
  return ''
}
...
export const name = '@goodandready/dsh-vision-bridge'
...
/**
 * Error code marking a fetch-policy refusal. Callers that treat per-source read
 * failures as skippable must re-throw this one: a refused hop is a security
 * decision, not a transient read error (#258 review).
 */
export const FETCH_POLICY_CODE = 'EFETCH_POLICY'
...
export async function safeFetch(raw, opts = {}) {
  ...
  for (let hop = 0; hop <= maxRedirects; hop++) {
    if (!(await isSafeFetchUrl(url, opts))) {
      throw Object.assign(new Error(`URL refused by fetch policy (#202): ${url}`), { code: FETCH_POLICY_CODE })
    }
 ...
 */
export async function stripEXIF(bytes, contentType, opts = {}) {
  let sharp = null
  try { sharp = (await import('sharp')).default } catch (err) { /* optional dependency probe */ void err }
  if (!sharp) {
    if (opts && opts.throwOnError) {
      const err = new Error('sharp unavailable')
      err.code = 'ERR_SHARP_UNAVAILABLE'
      throw err
    }
    return bytes
  }
 ...
 */
export async function deskewImage(bytes, contentType, opts = {}) {
  let sharp = null
  try { sharp = (await import('sharp')).default } catch (err) { /* optional dependency probe */ void err }
  if (!sharp) {
    if (opts && opts.throwOnError) {
 ...
 */
export async function enhanceImage(bytes, contentType, opts = {}) {
  let sharp = null
  try { sharp = (await import('sharp')).default } catch (err) { /* optional dependency probe */ void err }
  if (!sharp) {
    if (opts && opts.throwOnError) {
