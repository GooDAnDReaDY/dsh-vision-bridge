      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (req.method === 'POST' && !id) {
            let body = {}
            try { body = await new Promise((resolve) => { let c = ''; req.on('data', (d) => { c += d }); req.on('end', () => { try { resolve(JSON.parse(c)) } catch (err) { /* bestEffort JSON parse */ resolve({}) } }) }) } catch (err) { /* bestEffort body read */ void err }
