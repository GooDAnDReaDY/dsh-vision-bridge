...
export function registerDiagnosticRoutes(ctx, deps) {
  const { config, liveChannels, callVisionModelWithBytes, channelCircuitStates, journal, evidenceStore, pluginVersion, probeStorage, usageByChannel, lastRequests } = deps
  ...
  ctx.effect(
    () =>
      ctx.webServer.register({
        kind: 'exact',
        path: '/dsh-vision-bridge/telemetry',
        handler: async (req, res) => {
          const writeJson = (status, body) => {
            res.writeHead(status, { 'Content-Type': 'application/json' })
            res.end(JSON.stringify(body))
          ...
          for (const ch of list) {
            ...
            const stats = (usageByChannel && usageByChannel.get(key)) || { calls: 0, totalMs: 0, errors: 0, lastMs: 0 }
            const avgMs = stats.calls > 0 ? Math.round(stats.totalMs / stats.calls) : 0
            const successRate = stats.calls > 0 ? Number((((stats.calls - stats.errors) / stats.calls) * 100).toFixed(1)) : 100
            channelsReport.push({
              key,
              type: ch.type,
              model: ch.model || '',
              ...
              successRate,
              health: stats.errors > 2 && stats.errors === stats.calls ? 'down' : avgMs > 3000 ? 'degraded' : 'healthy'
            })
          }
          writeJson(200, {
            ...
            recentRequests: (lastRequests || []).slice(-10)
          })
        },
      }),
    'dsh-vision-bridge: /telemetry route',
  )
