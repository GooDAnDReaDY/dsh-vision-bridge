    ...
    ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const { tmpdir } = await import('node:os')
          const { join } = await import('node:path')
          const { writeFileSync, readFileSync, unlinkSync, readdirSync, existsSync } = await import('node:fs')
          const dir = tmpdir()
          ...
          for (let i = 0; i < frames.length; i++) {
            ...
            try { unlinkSync(frameFile) } catch {}
            ...
            if (id !== undefined) attachmentById.set(String(id), savedRef)
