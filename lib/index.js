...
/** The text marker replacing one image block the model cannot see. */
export function markerFor(attachment, name) {
  const id = attachment && (attachment.attachmentId ?? attachment.id)
  const label = (name && name.trim()) || (attachment && attachment.name) || 'изображение'
  if (id === undefined || id === '' || id === 'unknown') {
    return [
      {
        type: 'text',
        text: `[В сообщении есть изображение «${label}». Текущая модель не видит картинки — вызовите describe_image и передайте путь к файлу (если он известен) или вопрос, чтобы vision-модель его описала.]`,
      },
    ]
  }
  return [
    {
      type: 'text',
      text:
        `[В сообщении есть изображение «${label}» (attachment id: "${id}"). ` +
        `Текущая модель действительно не видит картинки. Чтобы понять её содержание, ` +
        `вызовите инструмент describe_image с attachmentIds: ["${id}"] и конкретным вопросом ` +
        `(например, «опиши эту картинку»). Описание вернёт отдельная vision-модель.]`,
    },
  ]
}
...
export function apply(ctx, config) {
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    const blocks = [
      { type: 'image', attachment: { attachmentId: hash, mediaType: contentType || 'image/png', bytes: bytes.length, width: 0, height: 0, name: hash } },
  ...
  ctx.tools.register(
    defineTool({
      name: 'describe_image',
      description:
        'Send an image to the configured vision model and return its description/answer. Use this whenever you receive a ' +
        '[изображение …] text marker instead of a real picture (you are a text-only model). ' +
        'Pass attachmentIds (ids from the marker) and/or paths (local file paths), plus an optional question.',
      parameters: {
        attachmentIds: { type: 'array', items: { type: 'string' }, description: 'Attachment ids of images in this conversation (from the [изображение …] marker).' },
        paths: { type: 'array', items: { type: 'string' }, description: 'Optional local file paths of images to look at.' },
        question: { type: 'string', description: 'Optional question about the image. Default: describe it.' },
      },
      output: {
        schema: {
          type: 'object',
