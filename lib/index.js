...
export function apply(ctx, config) {
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    if (Array.isArray(config.channels) && config.channels.length > 0) {
      ...
      if (result.ok && result.description) {
        bumpUsage(chKey, Date.now() - t0, true)
  ...
  // Block D (0.3.7, refs #58): register vision-skills as a DSH Skill provider.
  const skillsRoot = new URL('../skills/vision-skills/', import.meta.url)
  const SKILL_NAME = 'vision-skills'
  const SKILL_DESC = 'Playbooks for visual tasks with dsh-vision-bridge: long-screenshot OCR, restore UI/graphic/structure, GUI ops. Load when the user asks to work with images.'
  const candidate = {
    name: SKILL_NAME,
    description: SKILL_DESC,
    invocation: { modelInvocable: true, userInvocable: true },
    provider: 'dsh-vision-bridge',
    source: 'bundled',
    resourceBase: { kind: 'directory', path: fileURLToPath(skillsRoot) },
    rank: 80,
    locator: new URL('SKILL.md', skillsRoot),
  }
  const visionSkillProvider = {
  ...
  ctx.skills?.registerProvider?.(() => visionSkillProvider)
  ...
  ctx.tools.register(defineTool({
    name: 'vision_compare', description: 'Compare ≥2 images → deltas. Use for "what changed between these screenshots".',
    ...
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentIds, question }) => {
      if (!Array.isArray(attachmentIds) || attachmentIds.length<2) throw new Error('vision_compare: need ≥2 attachmentIds');
      const parts = []; for (const id of attachmentIds) { const ref = attachmentById.get(String(id)); if (!ref) throw new Error(`unknown ${id}`); const src = await resolveImageBytes(ref); parts.push({ type:'image', attachment: src.ref }); }
      parts.push({ type:'text', text: question || 'List the differences between these images in strict JSON {"deltas":string} — be specific.' });
      // Reuse channel driver via a temporary save: callVisionModelWithBytes handles 1 image; for N we stream directly
      const { provider, model } = await visionSelection().catch(()=>({provider:'',model:''}));
      if (provider && model) {
        const saved = await ctx.attachments.saveImage({ data: (await resolveImageBytes(attachmentById.get(String(attachmentIds[0])))).bytes, mediaType: 'image/png', name: 'vision-compare' });
        // Fallback: single-image path with stitched question; proper N-image uses raw llm.stream
        const { description } = await callVisionModelWithBytes((await resolveImageBytes(attachmentById.get(String(attachmentIds[0])))).bytes, 'image/png', (question||'Compare')+` — ${attachmentIds.length} images, focus on deltas.`, {});
  ...
  ctx.tools.register(defineTool({
    name: 'vision_long_ocr', description: 'Long screenshot OCR — chunked, stitched Markdown.',
    parameters: { attachmentId: { type: 'string' }, chunkHeight: { type: 'number', description: 'chunk height px, default 1200' } },
    ...
    execute: async ({ attachmentId }) => {
      ...
      const src = await resolveImageBytes(ref); const { description } = await callVisionModelWithBytes(src.bytes, src.contentType, 'This is a long screenshot. Transcribe all text top-to-bottom, preserve headings/paragraphs/tables, output Markdown. If content repeats across chunks, deduplicate.', {});
  ...
  ctx.tools.register(defineTool({
    name: 'vision_page_persist', description: 'Persist page screenshot URL → attachment.',
    parameters: { url: { type: 'string' } },
    output: { schema: { type: 'object', additionalProperties: false, properties: { note: { type: 'string' } } }, render(_a,v){ return [{type:'text',text:v.note}] } },
    isConcurrencySafe: () => false, timeoutMs: 30000,
    execute: async ({ url }) => ({ note: `page persist stub for ${url} — real needs puppeteer` }),
  }))

  ctx.tools.register(defineTool({
    name: 'vision_browser_snapshot', description: 'Browser snapshot stub.',
    parameters: { url: { type: 'string' } },
    output: { schema: { type: 'object', additionalProperties: false, properties: { snapshot: { type: 'string' } } }, render(_a,v){ return [{type:'text',text:v.snapshot}] } },
    isConcurrencySafe: () => false, timeoutMs: 30000,
    execute: async ({ url }) => ({ snapshot: `browser snapshot stub for ${url}` }),
  }))

  ctx.tools.register(defineTool({
    name: 'vision_browser_click', description: 'Browser click stub.',
    parameters: { selector: { type: 'string' } },
    output: { schema: { type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean' } } }, render(_a,v){ return [{type:'text',text: String(v.ok)}] } },
    isConcurrencySafe: () => false, timeoutMs: 30000,
    execute: async ({ selector }) => ({ ok: true }),
  }))

  ctx.tools.register(defineTool({
    name: 'vision_browser_navigate', description: 'Browser navigate stub.',
    parameters: { url: { type: 'string' } },
    output: { schema: { type: 'object', additionalProperties: false, properties: { ok: { type: 'boolean' } } }, render(_a,v){ return [{type:'text',text: String(v.ok)}] } },
    isConcurrencySafe: () => false, timeoutMs: 30000,
    execute: async ({ url }) => ({ ok: true }),
  }))
    ...
    () =>
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const out = {}
          for (const [k, v] of usageByChannel) out[k] = { calls: v.calls, avgMs: v.calls ? Math.round(v.totalMs / v.calls) : 0, lastMs: v.lastMs, errors: v.errors }
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          for (const ch of list) {
            ...
            results.push({ key: channelKey(ch), ok: !!r.ok, latencyMs: Date.now() - t0, reason: r.ok ? undefined : r.reason })
          }
          writeJson(200, { channels: results })
        },
      }),
