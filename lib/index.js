...
// requireScope hoisted
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    try {
      ...
      journalAdd({ ok: false, channel: provider + '/' + model, imageHash: contentHash(bytes), prompt: String(question).slice(0, 200), reason: String((e && e.message) || e).slice(0, 200) })
      throw e
    }
    if (text && key) {
    ...
    journalAdd({ ok: true, channel: provider + '/' + model, imageHash: contentHash(bytes), prompt: String(question).slice(0, 200), latencyMs: 0 })
