...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { bbox: { type: ['array','null'], items: { type: 'number' } }, description: { type: 'string' } } }, render(_a,v){ return [{type:'text',text: v.bbox ? `bbox ${JSON.stringify(v.bbox)} — ${v.description}` : `not found — ${v.description}`}] } },
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { bbox: { type: ['array','null'] }, note: { type: 'string' } } }, render(_a,v){ return [{type:'text',text: v.bbox ? `crop bbox ${JSON.stringify(v.bbox)} — ${v.note}` : v.note}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId, region }) => {
      ...
      let bbox = null; if (/^\s*\d/.test(region)) { const parts = region.split(/[,\s]+/).map(Number); if (parts.length===4 && parts.every((n)=>!isNaN(n))) bbox = parts; }
      ...
      return { bbox, note: bbox ? 'bbox ready — with sharp, PNG crop here' : 'region not found' };
    },
  }))
  ...
  ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { items: { type: 'array', items: { type: 'object', properties: { label: {type:'string'}, bbox:{type:'array',items:{type:'number'}} }, required:['label','bbox'] } }, raw: {type:'string'} } }, render(_a,v){ return [{type:'text',text: v.items.length ? JSON.stringify(v.items,null,2) : v.raw}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
