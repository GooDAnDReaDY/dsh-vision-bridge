...
const dims = imageDimensions(bytes)
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    // Pre-warm descriptions into cache so llm/stream has them ready
    await rewriteImagesDeep(messages, async (block) => {
    ...
    // Keep original decision.messages intact so Web UI renders the actual image attachment
    return decision
  })
