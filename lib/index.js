...
// requireScope hoisted
  ...
  const getLiveConfig = () => {
    ...
    } catch (err) { /* bestEffort snapshot read */ void err }
    return config
  }
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] compressImage fallback:', err?.message || err) }
  ...
  if (config.includeOAuthProviders !== false) {
    ...
    } catch (err) { /* bestEffort subscription accounts probe */ void err }
  }
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    } catch (err) { /* bestEffort last user text extraction */ void err }
  ...
  const liftAgentMask = (agent) => {
    ...
    if (prev && typeof prev.dispose === 'function') { try { prev.dispose() } catch (err) { /* bestEffort dispose */ void err } }
