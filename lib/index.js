...
// #285: write/read/delete a probe file in tmpdir() to verify the plugin can
// actually use the temporary storage that pdftoppm, ffmpeg and /upload-pdf
// depend on. Returns { ok, error } — called only with ?probe=1 on /doctor.
...
export function apply(ctx, config) {
  ...
  let batchSeq = 0
  // #236: how long a finished batch stays pollable before its record is released.
...
// requireScope hoisted
  ...
  const startBatch = async (items, prompt) => {
    ...
    ;(async () => {
      ...
      // #227: results stay pollable for the TTL, then the record (and the
      // descriptions it holds) is released instead of growing the Map forever.
      // #249: the timer must not keep the event loop alive — a short-lived host
      // (test runner, CLI) would otherwise wait the whole TTL before exiting.
      const rec = batches.get(id)
      if (!rec) return // released while still running: nothing left to schedule
  ...
  const resolveChannelForProvider = async (provider, model) => {
    try {
        ...
        if (found.apiKeyEnv) {
          try {
            ...
            if (typeof console !== 'undefined' && console.debug) {
              console.debug('[dsh-vision-bridge] resolveChannelForProvider:', credError)
            }
          }
      ...
      if (typeof console !== 'undefined' && console.debug) {
        console.debug('[dsh-vision-bridge] resolveChannelForProvider failed:', err?.message || err)
      }
    }
    return null
  }
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    if (config.deskew) {
      try {
        bytes = await deskewImage(bytes, contentType, { throwOnError: true })
      } catch (err) {
        preprocessWarnings.push(`deskew failed: ${err?.message || err}`)
      }
    }
    if (config.enhanceImage) {
      try {
        bytes = await enhanceImage(bytes, contentType, { throwOnError: true })
      } catch (err) {
        preprocessWarnings.push(`enhance failed: ${err?.message || err}`)
      }
    }
    // #203: stripEXIF now actually runs (sharp keeps only the pixel data).
    if (config.stripEXIF) {
      try {
        bytes = await stripEXIF(bytes, contentType, { throwOnError: true })
      } catch (err) {
        preprocessWarnings.push(`stripEXIF failed: ${err?.message || err}`)
      }
    }
    ...
    try {
      ...
      preprocessWarnings.push(`compressImage failed: ${err?.message || err}`)
      if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] compressImage fallback:', err?.message || err)
    }
    ...
    if (ph && descriptionByHash.has('ph:' + ph)) {
      ...
      return { description: cached, cached: true, warnings: preprocessWarnings }
    }
      const key = cacheKeyFor(bytes, question, modelHint)
      if (key) {
        const cached = descriptionByHash.get(key)
        if (typeof cached === 'string' && cached.trim()) return { description: cached, cached: true, warnings: preprocessWarnings }
        ...
        return { description: persisted, cached: true, warnings: preprocessWarnings }
      }
      }
      const t0 = Date.now()
      ...
      if (result.ok && result.description) {
        ...
        return { description: desc, cached: false, attempts: result.attempts, keyUsed: keyLabel, usage: result.usage, warnings: preprocessWarnings }
      }
  ...
  const describeAttachment = async (ref) => {
    ...
    if (config.describeStrategy === 'ocr-local') {
      ...
      if (!firstPass || !firstPass.trim()) {
        ...
        } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] firstPass OCR fallback:', err?.message || err) }
      }
    }
  ...
  // — Block 1 (0.2.3) Grounding suite — 5 tools, single vision call → JSON bbox
  // ponytail: without sharp — crop returns bbox; with sharp — real PNG (single point of replacement)
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    // #239: index attachment references regardless of the bridge gate — the
    // tools resolve images by id even in tools mode, where no rewrite runs.
    // Recording is a read-only map update; only the rewrite below is gated.
    ...
    // Capture the latest user text as a focus hint (bridge prompts only)
  ...
  const ATTACH_ONLY_TOOLS = ['vision_attach_pages', 'vision_attach_frames', 'vision_attach_images']
  ...
  const liftAgentMask = (agent) => {
    ...
    if (prev && typeof prev.dispose === 'function') { bestEffort('agentMask.dispose', () => prev.dispose()) }
  ...
  const applyAgentToolPolicy = async (agent, signal) => {
    ...
    if (prev && typeof prev.dispose === 'function') { bestEffort('agentMask.dispose', () => prev.dispose()) }
    agentToolPolicy.delete(agent)
    // #242/#241: the mask mirrors who the route is. Bridging (text-only, or a
    // forced bridge) -> hide the attach tools, which that model cannot use;
    // native passthrough -> hide the compensation tools it does not need.
    const deny = shouldBridgeForModel(config, supportsImages) ? ATTACH_ONLY_TOOLS : REDUNDANT_FOR_VISION
    if (deny.length === 0) { agentToolPolicy.set(agent, { key, dispose: null }); return }
    try {
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    const decision = await next()
    try { await applyAgentToolPolicy(payload && payload.agent, payload && payload.signal) } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] applyAgentToolPolicy failed:', err?.message || err) }
    return decision
  })
  ...
  // #291: HTTP route registrations moved to lib/routes/* domain files.
  const routesDeps = {
    config,
    requireScope,
    liveChannels,
  ...
  async function resolveSourceBytes(source, attachmentId, path) {
    ...
    if (!src) {
      ...
      if (lastRef) {
        ...
        } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] readImage fallback:', err?.message || err) }
      }
      return null;
    }
    if (/^https?:\/\//i.test(src)) {
    ...
    if (dshFs) {
      ...
      } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] dshFs readBytes fallback:', err?.message || err) }
    }
    ...
    } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] local file read fallback:', err?.message || err) }
    return null;
  }
  ...
  const toolsDeps = { ctx, config, attachmentById, descriptionByAttachmentId, descriptionByHash, batches, startBatch, callVisionModelWithBytes, visionSelection, resolveImageBytes, resolveSourceBytes, collectText, describeImage, effectivePrompt, liveChannels, groundingPrompt, parseBbox, tesseractAvailable, recordAttachment }
