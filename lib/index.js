...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    name: 'vision_html_screenshot', description: 'Render local HTML → screenshot (via vision LLM note; real render needs headless Chrome).',
    parameters: { path: { type: 'string', description: 'local .html file path' } },
    ...
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ path }) => {
      // ponytail: no headless Chrome dep — returns note only. Upgrade: puppeteer-core + saveImage.
      const fs = ctx.get('fs'); if (!fs) throw new Error('vision_html_screenshot: fs unavailable');
      await fs.resolve(path);
      return { note: 'html_screenshot stub — real PNG requires headless Chrome (puppeteer-core), planned in v2.0', attachmentId: '' };
    },
  }))
  ...
  ctx.tools.register(defineTool({
    name: 'vision_video_describe', description: 'Describe video content (stub — real needs frame extraction).',
    parameters: { path: { type: 'string' }, question: { type: 'string' } },
    ...
    execute: async ({ path, question }) => {
      // ponytail: no ffmpeg — stub. Upgrade: extract frames → vision call per frame
      return { description: `video stub for ${path}: ${question || 'describe'}` };
    },
  }))
