...
export const Config = z.object({
  ...
  selfCheckEnabled: z
    .boolean()
    .description('Enable vision_self_check tool for visual hypothesis verification.')
    .default(true),
  maskPII: z
    .boolean()
...
export function apply(ctx, config) {
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    if (config.tileLargeImages) {
      const tiles = await tileImage(bytes, contentType, { maxPixels: config.tileThreshold || 4000000 })
      if (tiles.length > 1) {
        // Process tiles sequentially and combine descriptions
        const descs = []
        for (const tile of tiles) {
          try {
            const r = await callVisionModelWithBytes(tile.bytes, tile.contentType, question + ' [tile ' + (tile.index+1) + '/' + tiles.length + ']', opts)
            if (r.description) descs.push(r.description)
          } catch {}
        }
        const combined = descs.filter(Boolean).join('\n\n')
        return { description: combined, cached: false, attempts: tiles.length }
      }
      bytes = tiles[0].bytes
      contentType = tiles[0].contentType
    }
...
const dims = imageDimensions(bytes)
            ...
            if (req.method === 'POST') {
              ...
              try {
                ...
                if (Object.prototype.hasOwnProperty.call(body, 'provider') && Object.prototype.hasOwnProperty.call(body, 'model')) {
                  ...
                  // If only one of provider/model is set, ignore (partial — don't clobber existing)
                }
                ...
                writeJson(200, { provider: outProvider, model: outModel, mode: incomingMode || 'hybrid', describeStrategy: incomingStrategy || 'auto', escalation: incomingEscalation || 'simple-only' })
              } catch (error) {
  ...
  // #186 workaround: inject client-side script to allow images for text-only models
  // The DSH composer checks inputModalities before sending; vision-bridge handles
  // the actual image processing via agent/pre-step and llm/stream hooks.
  ctx.effect(() => {
...
(function() {
  function patchFetch() {
    ...
    if (origFetch._visionBridgePatched) return;
    window.fetch = function(url, opts) {
      var result = origFetch.apply(this, arguments);
      if (typeof url === 'string' && url.includes('/llm/') && url.includes('/models/')) {
        return result.then(function(response) {
          if (response.ok) {
            return response.clone().json().then(function(data) {
              if (data && data.inputModalities && !data.inputModalities.includes('image')) {
                data.inputModalities.push('image');
    ...
    window.fetch._visionBridgePatched = true;
  }
  patchFetch();
  setTimeout(patchFetch, 2000);
  setTimeout(patchFetch, 5000);
})();
</script>`;
    ...
    return function() {}
  }, 'dsh-vision-bridge: tapIndex image validation bypass')
