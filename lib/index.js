        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            if (req.method === 'POST') {
              ...
              // #269: the attach cap is bounded in the schema too; reject out-of-range
              // input here so the card gets a clear message instead of a silent clamp.
              // #284: numeric settings are range-checked before the whitelist loop
              // so a bad value is rejected with a clear error, never stored.
              ...
              if (Object.prototype.hasOwnProperty.call(body, 'attachMaxItems')) {
                // #276: Number(true) === 1, so a coercion would let a boolean through.
              ...
              // #269 review: the toggle reaches the config snapshot through the same
              // whitelist, so a non-boolean value must not slip through.
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          for await (const chunk of req) {
            total += chunk.length
            if (tooLarge) {
              // #236 review: `break` here would call the async iterator's
              // return(), destroying the request before the client can read
              // the 413 — so keep reading and discard, with a hard ceiling so
              // a hostile body cannot be read forever (breaking there is the
  ...
  ctx.effect(
    () =>
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          const writeJson = (status, body) => { res.writeHead(status, { 'Content-Type': 'application/json' }); res.end(JSON.stringify(body)) }
          // #201: starting and cancelling batches spends vision calls, and
          // #249: releasing drops the record — same-origin only for all three.
      ...
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const summary = {
            ...
            // #285: surface the plugin version and storage health so a broken
            // deployment shows up in the first diagnostic, not as a mystery
            // EACCES on the first vision call.
