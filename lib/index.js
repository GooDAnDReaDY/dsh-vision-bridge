          ...
          try {
            ...
            if (typeof ctx.llm.listProviders === 'function') {
              for (const p of ctx.llm.listProviders() || []) {
                if (p && p.id) providerIds.add(p.id)
                if (p && p.provider) providerIds.add(p.provider)
              }
            }
            ...
            try {
              const modelsScope = ctx.settings && ctx.settings.get ? ctx.settings.get('models') : null
              if (modelsScope && typeof modelsScope === 'object') {
                for (const k of Object.keys(modelsScope)) {
                  if (k && typeof k === 'string') providerIds.add(k)
                }
              }
            } catch {}
