...
// requireScope hoisted
  ...
  const REDUNDANT_FOR_VISION = [
    ...
    'vision_ui_layout', 'vision_translate_image', 'vision_to_code',
  ...
  const applyAgentToolPolicy = async (agent, signal) => {
    if (config.hideRedundantTools === false || restrictUnavailable || !agent) return
    ...
    if (!supportsImages) { agentToolPolicy.set(agent, { key, dispose: null }); return }
    try {
