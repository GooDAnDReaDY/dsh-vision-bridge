...
export const Config = z.object({
  ...
  allowedUrlHosts: z
    .array(z.string())
    .description('If non-empty, only http(s) URLs on these hosts may be fetched server-side. Empty = any public host; private/loopback/link-local hosts are always refused.')
    .default([]),
...
const dims = imageDimensions(bytes)
  ...
  const describeImage = async (args, exec) => {
    ...
    for (const url of urls) {
      if (!(await isSafeFetchUrl(url, { allowedHosts: config.allowedUrlHosts }))) {
        throw new Error(`describe_image: URL refused by fetch policy (#202): ${url}`)
      }
  ...
  ctx.tools.register(
    defineTool({
      ...
      execute: async ({ source, question, detail }, exec) => {
        ...
        if (/^https?:\/\//i.test(src)) {
          if (!(await isSafeFetchUrl(src, { allowedHosts: config.allowedUrlHosts }))) {
            throw new Error(`inspect_image: URL refused by fetch policy (#202): ${src}`)
          }
  ...
  async function resolveSourceBytes(source, attachmentId, path) {
    ...
    if (/^https?:\/\//i.test(src)) {
      try {
        // #202: model-supplied URLs go through the same fetch policy.
        if (!(await isSafeFetchUrl(src, { allowedHosts: config.allowedUrlHosts }))) return null
        const res = await fetch(src, { signal: AbortSignal.timeout(Math.max(1000, config.channelTimeoutMs || 15000)) });
