...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    name: 'vision_batch', description: 'Process N images in parallel with the same prompt.',
    ...
    execute: async ({ attachmentIds, prompt }) => {
      ...
      const tasks = attachmentIds.map(async (id) => {
        try {
          const ref = attachmentById.get(String(id)); if (!ref) throw new Error(`unknown ${id}`)
          const src = await resolveImageBytes(ref)
          const r = await callVisionModelWithBytes(src.bytes, src.contentType, prompt || 'Describe this image.', {})
          return { id, description: r.description || '' }
        } catch (e) {
    ...
    () =>
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          const writeJson = (status, body) => { res.writeHead(status, { 'Content-Type': 'application/json' }); res.end(JSON.stringify(body)) }
          if (req.method !== 'GET') { writeJson(405, { error: 'method not allowed' }); return }
          // ponytail: rough estimate — avg image ≈ 1500 tokens in + 200 out. Prices are deployment-specific.
          const ASSUMED_IN = 1500, ASSUMED_OUT = 200
          const out = {}
          for (const [k, v] of usageByChannel) {
            out[k] = { calls: v.calls, estTokensIn: v.calls * ASSUMED_IN, estTokensOut: v.calls * ASSUMED_OUT, note: 'multiply by your provider price per token for actual cost' }
          }
          writeJson(200, { channels: out })
        },
      }),
