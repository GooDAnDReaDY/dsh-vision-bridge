...
export function apply(ctx, config) {
  ...
  const bumpUsage = (key, ms, ok, keyUsed) => {
  ...
  const callVisionModelWithBytes = async (bytes, contentType, question, opts) => {
    ...
    if (Array.isArray(config.channels) && config.channels.length > 0) {
      ...
      if (result.ok && result.description) {
        bumpUsage(chKey, Date.now() - t0, true, result.keyUsed)
        ...
        return { description: result.description, cached: false, attempts: result.attempts, keyUsed: keyLabel }
      }
    ...
    () =>
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const out = {}
          for (const [k, v] of usageByChannel) out[k] = { calls: v.calls, avgMs: v.calls ? Math.round(v.totalMs / v.calls) : 0, lastMs: v.lastMs, errors: v.errors, quota: v.quota || {} }
