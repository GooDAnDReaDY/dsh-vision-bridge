...
export function apply(ctx, config) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      schema: { type: 'object', description: 'JSON Schema for structured extraction (overrides format)' },
    },
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: { attachmentId: { type: 'string' } },
    output: { schema: { type: 'object', additionalProperties: false, properties: { codes: { type: 'array', items: { type: 'object', properties: { type: { type: 'string' }, data: { type: 'string' } } } } } }, render(_a,v){ return [{type:'text',text: v.codes.length ? JSON.stringify(v.codes, null, 2) : 'no codes found'}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId }) => {
