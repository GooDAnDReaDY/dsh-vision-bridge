...
import { analyzeImageQuality, computePixelDiff } from '../image-processing.js'
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentIdA, attachmentIdB, focus }, exec) => {
      ...
      try { const j = JSON.parse((description || '').match(/\{[\s\S]*\}/)?.[0] || ''); if (j.differences || j.summary) parsed = j } catch (err) { parseWarning = 'Failed to parse comparison JSON: ' + (err?.message || err) }
      const warnings = parseWarning ? [parseWarning] : []; return { ...parsed, warnings, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { plan: { type: 'string' }, analysis: { type: 'string' }, verification: { type: 'string' }, answer: { type: 'string' }, warnings: { type: 'array', items: { type: 'string' } }, parseWarning: { type: 'string' }, raw: { type: 'string' } } }, render(_a,v){ return [{type:'text',text: `Plan: ${v.plan}\n\nAnalysis: ${v.analysis}\n\nVerification: ${v.verification}\n\nAnswer: ${v.answer}`}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 30000,
    execute: async ({ attachmentId, question }, exec) => {
      ...
      let parseWarning = null
      try { const j = JSON.parse((description || '').match(/\{[\s\S]*\}/)?.[0] || ''); if (j.plan || j.analysis) parsed = j } catch (err) { parseWarning = 'Failed to parse CoT JSON: ' + (err?.message || err) }
      const warnings = parseWarning ? [parseWarning] : []; return { ...parsed, warnings, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
...
if (config.selfCheckEnabled) ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { confidence: { type: 'number' }, verified: { type: 'boolean' }, answer: { type: 'string' }, refined: { type: 'string' }, warnings: { type: 'array', items: { type: 'string' } }, parseWarning: { type: 'string' } } }, render(_a,v){ return [{type:'text',text: `Confidence: ${v.confidence}%, Verified: ${v.verified}\nAnswer: ${v.answer}${v.refined ? '\nRefined: ' + v.refined : ''}`}] } },
    ...
    execute: async ({ attachmentId, hypothesis, threshold }, exec) => {
      ...
      let parseWarning = null
      try { const j = JSON.parse((r1 || '').match(/\{[\s\S]*\}/)?.[0] || ''); if (typeof j.confidence === 'number') parsed = j } catch (err) { parseWarning = 'Failed to parse self_check JSON: ' + (err?.message || err) }
      let refined = ''
      ...
      const warnings = parseWarning ? [parseWarning] : []; return { confidence: parsed.confidence, verified: parsed.confidence >= (threshold || 70), answer: parsed.answer, refined, warnings, ...(parseWarning ? { parseWarning } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentIds, paths, title }, exec) => {
      ...
      const parsed = bestEffort('vision_ui_flow.parse', () => JSON.parse(description.match(/\{[\s\S]*\}/)?.[0] || ''), null);
      const warnings = parsed ? [] : ['Failed to parse UI flow steps JSON; fallback used'];
    ...
    execute: async ({ attachmentId, path, question, minAgreement }, exec) => {
      ...
      if (channels.length >= 2) {
        for (const ch of channels) {
          ...
          } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] multi-model channel query failed:', err?.message || err) }
        }
      }
      ...
      const parsed = bestEffort('vision_consensus.parse', () => JSON.parse(syn.match(/\{[\s\S]*\}/)?.[0] || ''), null);
      const warnings = parsed ? [] : ['Failed to parse consensus JSON; fallback used'];
...
ctx.tools.register(defineTool({
  ...
  ctx.tools.register(defineTool({
    name: 'vision_analyze_quality',
    description: 'Deterministically analyze image quality: blur score (Laplacian variance), mean brightness, RMS contrast, and 5-color dominant palette without LLM token cost.',
    parameters: {
      attachmentId: { type: 'string', description: 'Attachment ID or file path' },
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          dominantPalette: { type: 'array', items: { type: 'string' } },
          recommendation: { type: 'string' },
        },
      },
      render(_a, v) {
        return [{ type: 'text', text: `Quality: ${v.blurState} (blur score: ${v.blurScore}), Brightness: ${v.brightness}%, Contrast: ${v.contrast}, Palette: ${(v.dominantPalette||[]).join(', ')}` }]
      },
    },
    isConcurrencySafe: () => false,
    timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId, path: filePath }) => {
      const src = await resolveSourceBytes(null, attachmentId, filePath);
      if (!src) throw new Error('vision_analyze_quality: image source not found');
      const metrics = await analyzeImageQuality(src.bytes);
      const rec = metrics.blurState === 'blurry'
        ? 'Image appears out of focus or blurry. A clearer capture is recommended for fine details.'
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Deterministically compare two images pixel-by-pixel: calculates exact difference percentage and bounding box of changes without calling a vision model.',
    parameters: {
      attachmentIdA: { type: 'string', description: 'First image attachment ID or path' },
      attachmentIdB: { type: 'string', description: 'Second image attachment ID or path' },
      tolerance: { type: 'number', default: 30, description: 'RGB color difference threshold (0-255)' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          ...
          dimensionMismatch: { type: 'boolean' },
          changedBbox: { type: 'array', items: { type: 'number' } },
        },
      },
      render(_a, v) {
        return [{ type: 'text', text: v.isIdentical ? 'Images are pixel-identical' : `Images differ by ${v.diffPercentage}% (${v.diffPixels} px changed, bbox: ${JSON.stringify(v.changedBbox)})` }]
      },
    },
    isConcurrencySafe: () => false,
    timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentIdA, attachmentIdB, tolerance }) => {
      const srcA = await resolveSourceBytes(null, attachmentIdA);
      if (!srcA) throw new Error(`vision_pixel_diff: first image not found (${attachmentIdA})`);
      const srcB = await resolveSourceBytes(null, attachmentIdB);
      if (!srcB) throw new Error(`vision_pixel_diff: second image not found (${attachmentIdB})`);
      return await computePixelDiff(srcA.bytes, srcB.bytes, { tolerance });
    },
  }))
