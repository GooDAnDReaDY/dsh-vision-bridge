...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentIdA, attachmentIdB, focus }, exec) => {
      ...
      return { ...parsed, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentIds, paths, title }, exec) => {
      ...
      let parsed = null;
      try { parsed = JSON.parse(description.match(/\{[\s\S]*\}/)?.[0] || ''); } catch (err) { /* bestEffort JSON parse fallback */ void err }
...
if (config.consensusEnabled) ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, path, question, minAgreement }, exec) => {
      ...
      let parsed = null;
      try { parsed = JSON.parse(syn.match(/\{[\s\S]*\}/)?.[0] || ''); } catch (err) { /* bestEffort JSON parse fallback */ void err }
