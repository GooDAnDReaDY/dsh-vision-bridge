...
ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { text: { type: 'string' }, engine: { type: 'string' }, format: { type: 'string' }, warnings: { type: 'array', items: { type: 'string' } } } }, render(_a,v){ return [{type:'text',text:v.text}] } },
    ...
    execute: async ({ attachmentId, lang, engine, format, schema }) => {
      ...
      if (useEngine === 'tesseract' || (useEngine === 'auto' && (await tesseractAvailable()))) {
        try {
          ...
          try {
            ...
            bestEffort('ocr.unlink', () => unlinkSync(tmpFile))
          }
        } catch (err) { warnings.push('Tesseract OCR engine failed: ' + (err?.message || err) + '; fell back to vision LLM'); if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] tesseract ocr fallback:', err?.message || err) }
      }
      ...
      return { text: result, engine: usedEngine, format: format || 'text', warnings };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, psm, lang }) => {
      ...
      try {
        ...
        bestEffort('ocr.unlink', () => unlinkSync(inFile))
      }
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    parameters: { attachmentId: { type: 'string' } },
    output: { schema: { type: 'object', additionalProperties: false, properties: { result: { type: 'string' }, warnings: { type: 'array', items: { type: 'string' } } } }, render(_a,v){ return [{type:'text',text:v.result}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId }, exec) => {
      ...
      const parsed = bestEffort('vision_describe_structured.parse', () => JSON.parse((description || '').match(/\{[\s\S]*\}/)?.[0] || ''), null)
      ...
      return { result: parsed ? JSON.stringify(parsed, null, 2) : (description || ''), warnings }
    },
  }))
