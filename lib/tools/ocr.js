      ...
      if (useEngine === 'tesseract' || (useEngine === 'auto' && (await tesseractAvailable()))) {
        try {
          ...
          try {
            ...
            try { unlinkSync(tmpFile) } catch (err) { /* bestEffort cleanup */ void err }
          }
        } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] tesseract ocr fallback:', err?.message || err) }
      }
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, psm, lang }) => {
      ...
      try {
        ...
        try { unlinkSync(inFile) } catch (err) { /* bestEffort cleanup */ void err }
      }
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId }, exec) => {
      ...
      let parsed = null; try { parsed = JSON.parse((description || '').match(/\{[\s\S]*\}/)?.[0] || '') } catch (err) { /* bestEffort JSON parse fallback */ void err }
