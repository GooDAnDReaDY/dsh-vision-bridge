...
import { compressImage, isPathAllowed, isSafeFetchUrl, safeFetch, sniffMediaType } from '../vision-core.js'
...
export function registerAttachTools(d) {
  ...
  const publish = async (items, max) => {
    ...
    for (const item of kept) {
      ...
      const ref = await ctx.attachments.saveImage({ data: bytes, mediaType: contentType, name: item.name || 'attachment' })
    ...
    return { items: out, truncated, cap }
  }
  ...
  const renderAttachments = (v) => {
    ...
    blocks.push({ type: 'text', text: v.note || '' })
    return blocks
  }
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async ({ path, pages, maxItems }) => {
      ...
      const pdfPath = String(target && target.path !== undefined ? target.path : target)
      const dir = tmpdir()
      const stem = `vbap-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`
      const first = pages && /^\d+/.test(pages) ? Number(pages.split(/[-,]/)[0]) || 1 : 1
      ...
      const { items: published, truncated, cap: usedCap } = await publish(items, cap)
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async ({ path, frames, sceneDetect }) => {
      ...
      const target = await fs.resolve(path)
      const videoPath = String(target && target.path !== undefined ? target.path : target)
      const dir = tmpdir()
      ...
      const { items: published, truncated, cap: usedCap } = await publish(items, cap)
      return { items: published, truncated, note: `attached ${published.length} video frame(s) (cap ${usedCap}${truncated ? ', more frames sampled' : ''})` }
    },
  }))
    ...
    execute: async ({ paths, dirs, urls, maxItems }) => {
      ...
      for (const src of sources) {
        ...
        if (src.kind === 'dir') {
          ...
          const dirPath = String(target && target.path !== undefined ? target.path : target)
          let names = []
          try { names = readdirSync(dirPath).filter((f) => IMAGE_RE.test(f)).sort() } catch {}
          for (const name of names) {
            const full = join(dirPath, name)
            try {
              if (statSync(full).isFile()) items.push({ bytes: fs.readBytes ? await fs.readBytes(full, undefined, config.maxImageBytes) : readFileSync(full), name })
            } catch {}
            if (items.length > ATTACH_HARD_CAP) break
          }
          continue
        ...
        if (src.kind === 'path') {
          ...
          if (!isPathAllowed(src.value, config.allowedImageDirs)) throw new Error(`vision_attach_images: path ${src.value} is outside the allowedImageDirs`)
          const target = await fs.resolve(src.value)
          items.push({ bytes: await fs.readBytes(target, undefined, config.maxImageBytes), name: src.value.split(/[\\/]/).pop() })
          continue
        }
        ...
        const res = await safeFetch(src.value, {
          allowedHosts: config.allowedUrlHosts,
          init: { signal: AbortSignal.timeout(Math.max(1000, config.channelTimeoutMs || 15000)) },
        })
        if (!res.ok) throw new Error(`vision_attach_images: GET ${src.value} -> ${res.status}`)
        const declared = Number(res.headers.get('content-length') || 0)
        if (declared > config.maxImageBytes) throw new Error(`vision_attach_images: ${src.value} exceeds the ${config.maxImageBytes} byte limit`)
        const bytes = Buffer.from(await res.arrayBuffer())
        if (bytes.length > config.maxImageBytes) throw new Error(`vision_attach_images: ${src.value} exceeds the ${config.maxImageBytes} byte limit`)
        items.push({ bytes, contentType: res.headers.get('content-type') || sniffMediaType(bytes) || 'image/png', name: src.value.split('/').pop() || 'remote-image' })
      }
      const { items: published, truncated, cap: usedCap } = await publish(items, cap)
      if (published.length === 0) throw new Error('vision_attach_images: nothing to attach (no readable images found)')
      return { items: published, truncated, note: `attached ${published.length} image(s) (cap ${usedCap}${truncated ? ', more sources available' : ''})` }
    },
  }))
}
