...
export function resolvedPathOf(target, fs) {
  ...
  } catch (err) { /* bestEffort target path resolve */ void err }
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async ({ path, pages, maxItems }) => {
      ...
      try {
        ...
        for (const f of frames) { try { unlinkSync(join(dir, f)) } catch (err) { /* bestEffort cleanup */ void err } }
      }
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async ({ path, frames, sceneDetect }) => {
      ...
      try {
        ...
        for (const f of files) { try { unlinkSync(f) } catch (err) { /* bestEffort cleanup */ void err } }
      }
      const { items: published, truncated: publishTruncated, cap: usedCap, failed } = await publish(items, cap)
