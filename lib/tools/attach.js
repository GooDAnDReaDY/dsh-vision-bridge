...
export function resolvedPathOf(target, fs) {
  ...
  } catch (err) { bestEffort('attach.resolvePath', () => { throw err }) }
...
export function registerAttachTools(d) {
  ...
  const publish = async (items, max) => {
    ...
    for (const item of kept) {
      ...
      } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] attach compressImage failed:', err?.message || err) }
      let ref = null
      try {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async ({ path, pages, maxItems }) => {
      ...
      } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] pdftotext extraction failed:', err?.message || err) }
      const items = []
      try {
        ...
        for (const f of frames) { bestEffort('attach.unlinkFrame', () => unlinkSync(join(dir, f))) }
      }
    ...
    execute: async ({ paths, dirs, urls, maxItems }) => {
      ...
      for (const src of sources) {
        ...
        if (src.kind === 'dir') {
          ...
          let names = []
          try { names = readdirSync(dirPath).filter((f) => IMAGE_RE.test(f)).sort() } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] readdirSync failed:', err?.message || err) }
          for (const name of names) {
            ...
            } catch (err) { if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] attach readBytes failed for', full, err?.message || err) }
            if (items.length >= ATTACH_HARD_CAP) break
          }
          continue
        }
