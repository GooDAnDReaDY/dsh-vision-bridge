...
export function registerAttachTools(d) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async ({ path, pages, maxItems }) => {
      ...
      const { items: published, truncated, cap: usedCap, failed } = await publish(items, cap)
    ...
    execute: async ({ paths, dirs, urls, maxItems }) => {
      ...
      for (const src of sources) {
        ...
        if (remoteType && !/^image\//i.test(remoteType) && !sniffMediaType(bytes)) {
          throw new Error(`vision_attach_images: ${src.value} is not an image (content-type ${remoteType.slice(0, 40)})`)
        }
        items.push({ bytes, contentType: remoteType || sniffMediaType(bytes) || 'image/png', name: src.value.split('/').pop() || 'remote-image' })
      }
      const { items: published, truncated, cap: usedCap, failed } = await publish(items, cap)
