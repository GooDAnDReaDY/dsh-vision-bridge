...
ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { codes: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { type: { type: 'string' }, data: { type: 'string' } } } }, warnings: { type: 'array', items: { type: 'string' } }, parseWarning: { type: 'string' }, raw: { type: 'string' } } }, render(_a,v){ return [{type:'text',text: v.codes.length ? JSON.stringify(v.codes, null, 2) : 'no codes found'}] } },
    ...
    execute: async ({ attachmentId }) => {
      ...
      let codes = []; let parseWarning = null;
      try { const j = JSON.parse((description || '').match(/\{[\s\S]*\}/)?.[0] || ''); if (Array.isArray(j.codes)) codes = j.codes; } catch (err) { parseWarning = 'Failed to parse QR/barcode JSON: ' + (err?.message || err) }
      const warnings = parseWarning ? [parseWarning] : []; return { codes, warnings, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ path, pages, question }) => {
      ...
      for (const f of frames.sort()) {
        const bytes = readFileSync(join(dir, f)); bestEffort('vision_pdf_pages.unlink', () => unlinkSync(join(dir, f)))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, path, format }, exec) => {
      ...
      const { description, warnings: prepWarnings = [] } = await callVisionModelWithBytes(src.bytes, src.contentType, prompt, { ...(exec ? { signal: exec.signal } : {}) });
      const parsed = bestEffort('vision_extract_formula.parse', () => JSON.parse(description.match(/\{[\s\S]*\}/)?.[0] || ''), null);
      const warnings = [...prepWarnings];
      if (!parsed) warnings.push('Failed to parse structured JSON from vision response; raw output returned');
      return {
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, path, format }, exec) => {
      ...
      const { description, warnings: prepWarnings = [] } = await callVisionModelWithBytes(src.bytes, src.contentType, prompt, { ...(exec ? { signal: exec.signal } : {}) });
      const parsed = bestEffort('vision_extract_table.parse', () => JSON.parse(description.match(/\{[\s\S]*\}/)?.[0] || ''), null);
      const warnings = [...prepWarnings];
      if (!parsed) warnings.push('Failed to parse table JSON from vision response; raw output returned');
      return {
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, path, type }, exec) => {
      ...
      const { description, warnings: prepWarnings = [] } = await callVisionModelWithBytes(src.bytes, src.contentType, prompt, { ...(exec ? { signal: exec.signal } : {}) });
      const parsed = bestEffort('vision_scan_barcode.parse', () => JSON.parse(description.match(/\{[\s\S]*\}/)?.[0] || ''), null);
      const warnings = [...prepWarnings];
      if (!parsed) warnings.push('Failed to parse barcode JSON from vision response');
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, path, schema: schemaArg, documentType }, exec) => {
      ...
      const { description, warnings: prepWarnings = [] } = await callVisionModelWithBytes(src.bytes, src.contentType, prompt, { ...(exec ? { signal: exec.signal } : {}) });
      const parsed = bestEffort('vision_extract_structured.parse', () => JSON.parse(description.match(/\{[\s\S]*\}/)?.[0] || ''), null);
      const warnings = [...prepWarnings];
      if (!parsed) warnings.push('Failed to parse structured JSON from vision response; raw output returned');
      return {
  ...
  ctx.tools.register(defineTool({
    name: 'vision_export_artifact',
    description: 'Export extracted visual data (tables, formulas, diagrams, structured JSON) into persistent workspace files (.csv, .md, .tex, .mmd, .json).',
    parameters: {
      content: { type: 'string', description: 'Content string to export' },
      type: { type: 'string', enum: ['table', 'formula', 'diagram', 'json', 'text'], default: 'text', description: 'Artifact type' },
      filename: { type: 'string', description: 'Target filename (e.g. data.csv, math.tex, flow.mmd)' },
      format: { type: 'string', enum: ['csv', 'markdown', 'latex', 'mermaid', 'json', 'raw'], default: 'raw', description: 'Target file format' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render(_a, v) {
        return [{ type: 'text', text: `Exported ${v.type} artifact to ${v.filename} (${v.bytesWritten} bytes)` }];
      },
    },
    isConcurrencySafe: () => false,
    timeoutMs: config.timeoutMs + 10000,
    execute: async ({ content: textContent, type: artType, filename: userFilename, format }) => {
      if (typeof textContent !== 'string' || !textContent.trim()) {
        throw new Error('vision_export_artifact: content is required');
      }

      // Default filename based on type and format
      let finalName = userFilename;
      if (!finalName) {
        const ext = artType === 'table' ? (format === 'csv' ? 'csv' : 'md')
          : artType === 'formula' ? 'tex'
          : artType === 'diagram' ? 'mmd'
          : artType === 'json' ? 'json'
          : 'txt';
        finalName = `artifact-${Date.now()}.${ext}`;
      }
      ...
      // Resolve safe target path in project or tmp
      const targetDir = process.cwd();
      const resolvedPath = nodePath.resolve(targetDir, finalName);

      try {
        nodeFs.writeFileSync(resolvedPath, buf);
        return {
          success: true,
          filename: finalName,
          path: resolvedPath,
        ...
        throw new Error('vision_export_artifact write failed: ' + err.message);
      }
    },
  }))
