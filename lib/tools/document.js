...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId }) => {
      ...
      return { codes, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ path, pages, question }) => {
      ...
      for (const f of frames.sort()) {
        const bytes = readFileSync(join(dir, f)); try { unlinkSync(join(dir, f)) } catch (err) { /* bestEffort cleanup */ void err }
