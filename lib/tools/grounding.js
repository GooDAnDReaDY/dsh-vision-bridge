...
ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { items: { type: 'array', items: { type: 'object', properties: { label: {type:'string'}, bbox:{type:'array',items:{type:'number'}} }, additionalProperties: false } }, raw: {type:'string'}, warnings: { type: 'array', items: { type: 'string' } }, parseWarning: { type: 'string' } } }, render(_a,v){ return [{type:'text',text: v.items.length ? JSON.stringify(v.items,null,2) : v.raw}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId, kind }) => {
      ...
      const warnings = parseWarning ? [parseWarning] : []; return { items, raw: description || '', warnings, ...(parseWarning ? { parseWarning } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { svg: { type: 'string' }, warnings: { type: 'array', items: { type: 'string' } }, parseWarning: { type: 'string' }, raw: { type: 'string' } } }, render(_a,v){ return [{type:'text',text:v.svg.slice(0,500)}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId }) => {
      ...
      let svg = ''; let parseWarning = null; try { const j = JSON.parse(description.match(/\{[\s\S]*\}/)?.[0]||''); svg = j.svg || ''; } catch (err) { parseWarning = 'Failed to parse trace JSON: ' + (err?.message || err) } const warnings = parseWarning ? [parseWarning] : []; return { svg, warnings, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
...
ctx.tools.register(defineTool({
    ...
    output: { schema: { type: 'object', additionalProperties: false, properties: { palette: { type: 'array', items: { type: 'string' } }, warnings: { type: 'array', items: { type: 'string' } }, parseWarning: { type: 'string' }, raw: { type: 'string' } } }, render(_a,v){ return [{type:'text',text:JSON.stringify(v.palette)}] } },
    isConcurrencySafe: () => false, timeoutMs: config.timeoutMs + 15000,
    execute: async ({ attachmentId, top }) => {
      ...
      let palette = []; let parseWarning = null; try { const j = JSON.parse(description.match(/\{[\s\S]*\}/)?.[0]||''); if (Array.isArray(j.palette)) palette = j.palette; } catch (err) { parseWarning = 'Failed to parse colors JSON: ' + (err?.message || err) } const warnings = parseWarning ? [parseWarning] : []; return { palette, warnings, ...(parseWarning ? { parseWarning, raw: description || '' } : {}) };
    },
  }))
