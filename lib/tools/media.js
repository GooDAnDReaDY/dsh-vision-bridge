...
ctx.tools.register(defineTool({
    ...
    execute: async ({ path, width, fullPage }) => {
      ...
      try { unlinkSync(out) } catch (err) { /* bestEffort cleanup */ void err }
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ path, question, frames, sceneDetect }) => {
      ...
      for (const f of outFrames) {
        const bytes = readFileSync(f); try { unlinkSync(f) } catch (err) { /* bestEffort cleanup */ void err }
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ url, width }) => {
      ...
      const bytes = readFileSync(out); try { unlinkSync(out) } catch (err) { /* bestEffort cleanup */ void err }
