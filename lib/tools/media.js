...
ctx.tools.register(defineTool({
    ...
    execute: async ({ path, width, fullPage }) => {
      ...
      bestEffort('media.unlink', () => unlinkSync(out))
...
ctx.tools.register(defineTool({
    ...
    execute: async ({ attachmentId, filename }) => {
      ...
      try {
        ...
        if (typeof console !== 'undefined' && console.debug) console.debug('[dsh-vision-bridge] fs.writeBytes fallback:', err?.message || err);
        const { writeFileSync } = await import('node:fs');
        writeFileSync(targetPath, src.bytes);
      }
