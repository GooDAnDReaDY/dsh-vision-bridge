...
function mergeSignal(timeoutMs, callSignal) {
  const ms = Math.max(1, timeoutMs || 30000);
  if (typeof AbortSignal.any === 'function') {
    return callSignal
      ? AbortSignal.any([AbortSignal.timeout(ms), callSignal])
      : AbortSignal.timeout(ms);
  }
  const ctrl = new AbortController();
  const t = setTimeout(() => { try { ctrl.abort(new Error('timeout')); } catch (err) { /* bestEffort abort */ void err } }, ms);
  if (callSignal) {
    if (callSignal.aborted) {
      clearTimeout(t);
      ctrl.abort(callSignal.reason);
    } else {
      callSignal.addEventListener('abort', () => {
        clearTimeout(t);
        try { ctrl.abort(callSignal.reason); } catch (err) { /* bestEffort abort */ void err }
      }, { once: true });
    }
  }
...
async function collectStreamDeltas(res) {
  ...
  try {
    ...
    try { reader.releaseLock() } catch (err) { /* bestEffort reader release */ void err }
  }
...
export function channelKey(channel) {
  ...
  if (t === 'vllm' || t === 'sglang') return t + ':' + (channel.baseURL || '') + '/' + (channel.model || '')
  return t + ':?'
}
