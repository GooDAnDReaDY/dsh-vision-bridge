...
function mergeSignal(timeoutMs, callSignal) {
  ...
  const t = setTimeout(() => { bestEffort('channel.timeoutAbort', () => ctrl.abort(new Error('timeout'))) }, ms);
  if (callSignal) {
    if (callSignal.aborted) {
      ...
      callSignal.addEventListener('abort', () => {
        clearTimeout(t);
        bestEffort('channel.signalAbort', () => ctrl.abort(callSignal.reason))
      }, { once: true });
    }
  }
...
async function collectStreamDeltas(res) {
  ...
  try {
    ...
    bestEffort('channel.readerRelease', () => reader.releaseLock())
  }
